...
def test_atom_attention_encoder(sc: Scenario):
    ...
    a = torch.randn(bs * sc.multiplicity, sc.n_res, 2 * sc.token_s).to(device) / 1000.0
    q = torch.randn(bs * sc.multiplicity, sc.n_atoms, sc.dim).to(device) / 1000.0
    c = torch.randn(bs, sc.n_atoms, sc.dim).to(device) / 1000.0
    atom_dec_bias = torch.randn(bs, sc.n_atoms, H, sc.heads * sc.depth).to(device) / 1000.0
    ...
    dim_single_cond = ref_module.atom_decoder.diffusion_transformer.layers[0].adaln.dim_single_cond
    ...
    weights_and_biases = create_atom_attention_decoder_weights(from_ref=ref_module)
    load_atom_attention_decoder_weights_torch(model, weights_and_biases)

    with torch.no_grad():
        ref_output_float = ref_module(atom_to_token, atom_mask, a, q, c, atom_dec_bias, sc.multiplicity, attn_metadata)
        ...
        if dtype != torch.float32:
            ...
            ref_output = ref_module(atom_to_token, atom_mask, a, q, c, atom_dec_bias, sc.multiplicity, attn_metadata)
    ...
    ref_output_float = ref_output_float.view(bs, sc.multiplicity, sc.n_atoms, -1)
