...
class RefSwiGLUTransitionFromOF3OSS(OF3OSS_SwiGLUTransition):
    ...
    def load_weights(cls,
        ...
        m = cls(c_in=c_in, n=n)
        layers = [
            m.layer_norm, m.swiglu.linear_a, m.swiglu.linear_b, m.linear_out
        ]
...
class RefMSAModuleBlockFromOF3OSS(OF3OSS_MSAModuleBlock):
    ...
    def load_weights(cls,
                     model: str = "openfold3",
                     layer_path: str = "msa_module.blocks.0",
                     state_dict: dict = None) -> 'RefMSAModuleBlockFromOF3OSS':
        if state_dict is None:
            state_dict = load_weights(model, local_files_only=False)
        ...
        tri_mul_out = RefTriangleMultiplicationFromOF3OSS.load_weights(
            model=model,
            layer_path=f"{layer_path}.pair_stack.tri_mul_out",
            state_dict=state_dict,
            _outgoing=True)
        tri_mul_in = RefTriangleMultiplicationFromOF3OSS.load_weights(
            model=model,
            layer_path=f"{layer_path}.pair_stack.tri_mul_in",
            state_dict=state_dict,
            _outgoing=False)
