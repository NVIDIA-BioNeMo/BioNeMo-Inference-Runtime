...
def test_calculate_ids_hit(query_aln, template_aln, expected_query_ids,
                           expected_template_ids):
    query_ids, template_ids = calculate_ids_hit(_chars(query_aln),
                                                _chars(template_aln))
    np.testing.assert_array_equal(
        query_ids, np.asarray(expected_query_ids, dtype=np.int64))
    np.testing.assert_array_equal(
        template_ids, np.asarray(expected_template_ids, dtype=np.int64))
...
def test_calculate_ids_hit_drops_double_gap_columns():
    ...
    np.testing.assert_array_equal(query_ids, np.asarray([1, 2],
                                                        dtype=np.int64))
    np.testing.assert_array_equal(template_ids,
...
def test_seq_identity_and_coverage(query_aln, template_aln, query_seq,
                                   expected_identity, expected_coverage):
    identity, coverage = seq_identity_and_coverage(_chars(query_aln),
                                                   _chars(template_aln),
                                                   query_seq)
...
def test_seq_identity_and_coverage_all_gap_query_is_zero():
    identity, coverage = seq_identity_and_coverage(_chars("--"), _chars("MR"),
                                                   "MR")
    assert identity == 0.0
    assert coverage == 0.0
