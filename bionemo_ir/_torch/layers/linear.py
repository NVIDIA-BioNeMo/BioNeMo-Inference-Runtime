...
#
# Adapted from the Apache-2.0 fused-linear layer in NVIDIA TensorRT-LLM:
# https://github.com/NVIDIA/TensorRT-LLM/blob/main/tensorrt_llm/_torch/modules/linear.py
# TensorRT-LLM is not a dependency of this package; only the layout is reused.
...
# a `torch.device(...)` call evaluated once at import time (B008).
...
class Linear(nn.Module):
    def __init__(
        ...
        weights_loading_config: WeightsLoadingConfig | None = None,
    ...
    def create_weights(self):
        ...
        self.weight = Parameter(torch.empty(weight_shape, dtype=self.dtype, device=device), requires_grad=False)

        if self.has_bias:
            self.bias = Parameter(
                torch.empty((self.out_features,), dtype=self.dtype, device=device), requires_grad=False
            )
        else:
