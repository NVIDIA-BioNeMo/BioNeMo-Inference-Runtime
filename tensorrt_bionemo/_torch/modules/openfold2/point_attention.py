...
class PointProjection(nn.Module):
    ...
    def forward(
        ...
        out_shape = points_local.shape[:-1] + (self.no_heads, self.num_points,
                                               3)

        if self.is_multimer:
            points_local = points_local.view(points_local.shape[:-1] +
                                             (self.no_heads, -1))

        points_local = torch.split(points_local,
                                   points_local.shape[-1] // 3,
                                   dim=-1)
...
class InvariantPointAttention(nn.Module):
    ...
    def forward(self, s: torch.Tensor, z: torch.Tensor, r: Union[Rigid,
                                                                 Rigid3Array],
        ...
        o_pt = [
            torch.matmul(a, v.to(a.dtype)) for v in torch.unbind(v_pts, dim=-3)
        ]
