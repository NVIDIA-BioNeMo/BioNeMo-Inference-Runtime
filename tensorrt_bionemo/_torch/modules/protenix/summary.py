...
def get_bin_centers(min_bin: float, max_bin: float,
...
def logits_to_score(logits: torch.Tensor,
                    min_bin: float,
                    max_bin: float,
                    no_bins: int,
    ...
    bin_centers = get_bin_centers(min_bin, max_bin,
                                  no_bins).to(logits.device, logits.dtype)
    score = prob @ bin_centers
    return (score, prob) if return_prob else score


def compute_contact_prob(distogram_logits: torch.Tensor,
                         min_bin: float,
                         max_bin: float,
                         no_bins: int,
                         thres: float = 8.0) -> torch.Tensor:
...
def _remap_contiguous(asym_id: torch.Tensor) -> torch.Tensor:
    ...
    if len(unique) != asym_id.max() + 1:
        ...
        asym_id = torch.tensor([remap[int(x)] for x in asym_id],
                               dtype=torch.long,
                               device=asym_id.device)
    return asym_id


def _prepare_ptm(pae_prob: torch.Tensor,
...
def calculate_ptm(pae_prob: torch.Tensor,
                  has_frame: torch.Tensor,
                  min_bin: float,
                  max_bin: float,
                  no_bins: int,
    ...
    prepared, early = _prepare_ptm(pae_prob, has_frame, min_bin, max_bin,
                                   no_bins, token_mask)
    if prepared is None:
        return early
...
def calculate_iptm(pae_prob: torch.Tensor,
                   has_frame: torch.Tensor,
    ...
    prepared, early = _prepare_ptm(pae_prob, has_frame, min_bin, max_bin,
                                   no_bins, token_mask, asym_id)
    if prepared is None:
        return early
    token_token_ptm, has_frame, asym_id = prepared
    is_diff_chain = asym_id[None, :] != asym_id[:, None]
    iptm = (token_token_ptm *
            is_diff_chain).sum(dim=-1) / (eps + is_diff_chain.sum(dim=-1))
    return iptm[..., has_frame].max(dim=-1).values


def calculate_chain_based_ptm(pae_prob: torch.Tensor, has_frame: torch.Tensor,
                              asym_id: torch.Tensor,
                              token_is_ligand: torch.Tensor, min_bin: float,
                              max_bin: float, no_bins: int) -> dict:
    ...
    n_chain = len(masks)
    chain_is_ligand = {
        a: token_is_ligand[m].sum() >= m.sum() // 2
    ...
    for a1 in range(n_chain):
        for a2 in range(n_chain):
            ...
            chain_pair_iptm[..., a1, a2] = calculate_iptm(
                ...
                asym_id,
                *args,
                token_mask=masks[a1] + masks[a2])

    chain_ptm = torch.zeros(batch + (n_chain, ), device=device)
    for a, m in masks.items():
        chain_ptm[..., a] = calculate_ptm(pae_prob,
                                          has_frame,
                                          *args,
                                          token_mask=m)

    chain_has_frame = [
        bool((masks[i] * has_frame).any()) for i in range(n_chain)
    ]
    chain_iptm = torch.zeros(batch + (n_chain, ), device=device)
    for a in range(n_chain):
        pairs = [(i, j) for i in range(n_chain) for j in range(n_chain)
                 if (i == a or j == a) and i != j and chain_has_frame[i]]
        if pairs:
            chain_iptm[...,
                       a] = torch.stack(
                           [chain_pair_iptm[..., i, j] for i, j in pairs],
                           dim=-1).mean(dim=-1)

    chain_pair_iptm_global = torch.zeros(batch + (n_chain, n_chain),
                                         device=device)
    for a1 in range(n_chain):
        for a2 in range(n_chain):
            if a1 == a2:
...
def calculate_chain_based_gpde(token_pair_pde: torch.Tensor,
...
def calculate_chain_based_plddt(atom_plddt: torch.Tensor,
                                asym_id: torch.Tensor,
                                atom_to_token_idx: torch.Tensor) -> dict:
...
def calculate_clash(pred_coordinate: torch.Tensor, asym_id: torch.Tensor,
                    atom_to_token_idx: torch.Tensor, is_polymer: torch.Tensor,
    ...
    has_clash = torch.zeros(n_sample,
                            n_chain,
                            n_chain,
...
class ProtenixConfidenceSummary(nn.Module):
    ...
    def contact_probs(self, distogram_logits: torch.Tensor) -> torch.Tensor:
        ...
        """
        return compute_contact_prob(distogram_logits.float(),
                                    *tuple(self.config.distogram_bins),
                                    thres=self.config.contact_threshold)

    @staticmethod
    def token_is_ligand(asym_id: torch.Tensor, atom_to_token_idx: torch.Tensor,
                        is_polymer: torch.Tensor) -> torch.Tensor:
        """Per-token ligand flag (a token is ligand if any of its atoms is)."""
        atom_is_ligand = (1 - is_polymer).long()
        return torch.zeros_like(asym_id).scatter_add(0, atom_to_token_idx,
                                                     atom_is_ligand) > 0
    ...
    def summary_one_sample(
        ...
        token_pair_pae, pae_prob = logits_to_score(pae_logits.float(),
                                                   *pae_bins,
                                                   return_prob=True)

        summary: dict[str, Any] = {}
        summary["plddt"] = atom_plddt.mean(dim=-1) * 100
        summary["gpde"] = (token_pair_pde * contact_probs).sum(
            dim=[-1, -2]) / contact_probs.sum(dim=[-1, -2])
        summary["ptm"] = calculate_ptm(pae_prob, has_frame, *pae_bins)
        summary["iptm"] = calculate_iptm(pae_prob, has_frame, asym_id,
                                         *pae_bins)
        summary.update(
            calculate_chain_based_gpde(token_pair_pde, contact_probs, asym_id))
        summary.update(
            calculate_chain_based_ptm(pae_prob, has_frame, asym_id,
                                      token_is_ligand, *pae_bins))
        summary.update(
            calculate_chain_based_plddt(atom_plddt, asym_id,
                                        atom_to_token_idx))
        summary["has_clash"] = calculate_clash(coordinate.unsqueeze(0),
                                               asym_id, atom_to_token_idx,
                                               is_polymer,
                                               cfg.af3_clash_threshold)[0]
        summary["disorder"] = torch.zeros_like(summary["ptm"])
        summary["num_recycles"] = torch.tensor(num_recycles,
                                               device=coordinate.device)
        summary["ranking_score"] = (cfg.iptm_weight * summary["iptm"] +
    ...
    def forward(self,
                ...
                is_polymer: torch.Tensor,
                num_recycles: int,
                return_full_data: bool = True) -> dict[str, Any]:
        ...
        token_is_ligand = self.token_is_ligand(asym_id, atom_to_token_idx,
                                               is_polymer)
        ...
        for i in range(pae_logits.shape[0]):
            summary_i, full_i = self.summary_one_sample(
                contact_probs, plddt_logits[i], pae_logits[i], pde_logits[i],
                coordinate[i], asym_id, has_frame, atom_to_token_idx,
                is_polymer, token_is_ligand, num_recycles, return_full_data)
