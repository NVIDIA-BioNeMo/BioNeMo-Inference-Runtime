...
from bionemo_ir._torch.layers.distogram import DistogramModule
...
from bionemo_ir._torch.modules.boltz.confidence import Boltz1ConfidenceModule
from bionemo_ir._torch.modules.boltz.confidence_utils import compute_contact_prob
from bionemo_ir._torch.modules.boltz.embedders import Boltz1InputEmbedder
...
class Boltz1(nn.Module, OptimizedModuleSetterMixin):
    ...
    def __init__(self, config: BaseConfig = None, include_load_weights: bool = True, model_name: str | None = None):
        ...
        ### Input projections ###
        s_input_dim = self.config.token_s + 2 * num_tokens + 1 + num_pocket_contact_info
        self.s_init = Linear(
            s_input_dim, self.config.token_s, bias=False, dtype=self.input_embedder_dtype, skip_create_weights=False
        )
        self.z_init_1 = Linear(
            s_input_dim, self.config.token_z, bias=False, dtype=self.input_embedder_dtype, skip_create_weights=False
        )
        ...
        diffusion_module = DiffusionModule(config=self.structure_module_config.score_model)
        self.diffusion_sampler = BoltzDiffusionSampler(
            config=self.structure_module_config, diffusion_module=diffusion_module
        )

        ### Confidence module ###
        self.confidence_module = Boltz1ConfidenceModule(self.confidence_module_config)
    ...
    def diffusion_module(self) -> DiffusionModule:
        """Model-owned denoiser exposed through the composed sampler."""
    ...
    def load_weights(self, weights: dict = None):
        ...
        trunk_weights["msa_module"] = convert_hf_msa_module_torch(
            config=msa_module_config, weights=weights, model_name=self.model_name
        )
        trunk_weights["pairformer_module"] = convert_hf_pairformer_torch(
            config=pairformer_config, weights=weights, model_name=self.model_name
        )
        ...
        diffusion_sampler_weights = convert_hf_structure_module_torch(
            config=self.structure_module_config, weights=weights, model_name=self.model_name
        )
        self.diffusion_sampler.load_weights(diffusion_sampler_weights)

        # Load weights for confidence module
        confidence_module_weights = convert_hf_confidence_torch(
            config=self.confidence_module_config, weights=weights, model_name=self.model_name
        )
    ...
    def forward(
        ...
        diffusion_output = self.diffusion_sampler(
            s_trunk=s.float(),
        ...
        x_pred = diffusion_output["sample_atom_coords"]
        s_diffusion = diffusion_output["diff_token_repr"] if self.confidence_module_config.use_s_diffusion else None
        ...
        ret = {
            "confidence_score": (4 * confidence_module_output["complex_plddt"] + iptm_score) / 5,
