...
def rna_only_folding(
    *,
    ...
    # Unknown ribonucleotide: ``RX`` internally, ``N`` in PDB/CIF — so
    # ``list(sequence)`` below already gives the right residue name.
    short_to_restype = {"A": "RA", "G": "RG", "C": "RC", "U": "RU", "N": "RX"}
...
def dna_only_folding(
    *,
    ...
    """DNA-only chain. Restypes DA/DG/DC/DT, plus DX for an unknown base."""
    res_idx_map, _ = _of3_indices()
    short_to_index = {"A": "DA", "G": "DG", "C": "DC", "T": "DT", "N": "DX"}
    # Short code and PDB/CIF name coincide for the canonical bases but diverge
    # for the unknown one: ``DX`` vs ``DN``.
    short_to_cif = {"A": "DA", "G": "DG", "C": "DC", "T": "DT", "N": "DN"}
    res_indices = [res_idx_map[short_to_index[s]] for s in sequence]
    per_res_atoms = [["P", "C5'", "C4'", "C3'", "O3'", "C1'"]] * len(sequence)
    residue_names = [short_to_cif[s] for s in sequence] if with_residue_names else None
...
def protein_only_folding(
    *,
    sequence: str = "AN",
    chain_index: int = 0,
    with_mol_types: bool = False,
) -> FoldingOutput:
    """Protein-only chain. Restypes are 1-letter amino acids (``N`` = Asn)."""
    res_idx_map, _ = _of3_indices()
    res_indices = [res_idx_map[s] for s in sequence]
    per_res_atoms = [["N", "CA", "C", "O", "CB"]] * len(sequence)
    mol_types = [0] * len(sequence) if with_mol_types else None
    return _build_folding(
        res_indices=res_indices,
        residue_indices=list(range(1, len(sequence) + 1)),
        chain_indices=[chain_index] * len(sequence),
