...
from tensorrt_llm_lite.functional import Tensor, concat, shape
...
def test_dynamic_const_tensor(dtype, input_shape):
    ...
    with tensorrt_llm_lite.net_guard(net):
        ...
        two_out.mark_output("two_out",
                            tensorrt_llm_lite.str_dtype_to_trt(dtype))
        one_out.mark_output("one_out",
                            tensorrt_llm_lite.str_dtype_to_trt(dtype))
        zero_out.mark_output("zero_out",
                             tensorrt_llm_lite.str_dtype_to_trt(dtype))
