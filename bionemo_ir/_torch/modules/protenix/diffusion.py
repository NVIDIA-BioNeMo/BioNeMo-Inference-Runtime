...
from bionemo_ir._torch.modules.protenix._common import DIFFUSION_CONSUMED_FEATURES, atom_encoder_kwargs
from bionemo_ir._torch.modules.protenix.atom_attention import (
    ProtenixAtomAttentionDecoder,
    ProtenixAtomAttentionEncoder,
)
from bionemo_ir._torch.sampling.contracts import SamplingContext
from bionemo_ir._torch.sampling.edm import (
    AF3EDMIntegrator,
    EDMIntegratorConfig,
    EDMRolloutPlan,
    EDMScheduleConfig,
...
class ProtenixDiffusionConditioning(nn.Module):
    ...
    def __init__(self, config: BaseConfig) -> None:
        ...
        self.relpe = RelativePositionEncoder(
            token_z=rc.c_z,
            r_max=rc.r_max,
            s_max=rc.s_max,
        ...
        self.layernorm_z = nn.LayerNorm(2 * c_z, bias=False, eps=eps, dtype=dtype)
        self.linear_no_bias_z = Linear(2 * c_z, c_z, bias=False, dtype=dtype, skip_create_weights=skip)
        ...
        # Single path.
        self.layernorm_s = nn.LayerNorm(c_s + c_s_inputs, bias=False, eps=eps, dtype=dtype)
        self.linear_no_bias_s = Linear(c_s + c_s_inputs, c_s, bias=False, dtype=dtype, skip_create_weights=skip)
        self.fourier_embedding = FourierEmbedding(c_noise, dtype=dtype)
        self.layernorm_n = nn.LayerNorm(c_noise, bias=False, eps=eps, dtype=dtype)
        self.linear_no_bias_n = Linear(c_noise, c_s, bias=False, dtype=dtype, skip_create_weights=skip)
        self.transition_s = nn.ModuleList(
            [Transition(dim=c_s, hidden=2 * c_s, eps=eps, dtype=dtype, skip_create_weights=skip) for _ in range(2)]
        )
    ...
    def _joint_layernorm_linear_z(self, z, relpe):
        ...
        var_r, mean_r = torch.var_mean(relpe, dim=-1, correction=0, keepdim=True)
    ...
    def prepare_pair(self, relp: torch.Tensor, z_trunk: torch.Tensor) -> torch.Tensor:
...
class ProtenixDiffusionModule(nn.Module):
    ...
    def __init__(self, config: BaseConfig) -> None:
        ...
        self.diffusion_conditioning = ProtenixDiffusionConditioning(config.diffusion_conditioning_config)
        self.atom_attention_encoder = ProtenixAtomAttentionEncoder(config.atom_encoder_config)
        ...
        self.linear_no_bias_s = Linear(c_s, c_token, bias=False, dtype=dtype, skip_create_weights=skip)
        # Token transformer keeps its own dtype (bf16 + auto pairwise backend
        # while the rest stays fp32); only skip is propagated.
        ttc = config.token_transformer_config.model_copy(update={"skip_create_weights": skip})
        self.diffusion_transformer = ProtenixDiffusionTransformer(ttc)
        self._token_dtype = ttc.torch_dtype
        self.layernorm_a = nn.LayerNorm(c_token, bias=False, eps=eps, dtype=dtype)
        self.atom_attention_decoder = ProtenixAtomAttentionDecoder(config.atom_decoder_config)
    ...
    def f_forward(
        ...
        if cache is not None:
            ...
            if attn_metadata is None:
                ...
                attn_metadata = self.atom_attention_encoder.atom_transformer.build_attn_metadata(
                    K, self.n_queries, self.n_keys, r_noisy.device
                )

            s_single, z_pair = self.diffusion_conditioning(
                t_hat_noise_level, input_feature_dict["relp"], s_inputs, s_trunk, z_trunk
            )
    ...
    def forward(
        ...
        r_update = self.f_forward(
            r_noisy, t_hat_noise_level, input_feature_dict, s_inputs, s_trunk, z_trunk, attn_metadata, cache
        )
        s_ratio = (t_hat_noise_level / self.sigma_data)[..., None, None].to(r_update.dtype)
        x_denoised = (
            1 / (1 + s_ratio**2) * x_noisy + t_hat_noise_level[..., None, None] / torch.sqrt(1 + s_ratio**2) * r_update
        ).to(r_update.dtype)
        return x_denoised


class ProtenixDiffusionSampler(nn.Module):
    """Protenix denoiser adapter composed with the shared AF3 EDM runtime."""
    ...
    def noise_schedule(
        self, n_step: int | None = None, device: torch.device | None = None, dtype: torch.dtype = torch.float32
    ) -> torch.Tensor:
        """AF3 inference noise schedule (``[n_step + 1]``, final step -> 0)."""
        return EDMScheduleConfig(
            sigma_data=self.diffusion_module.sigma_data,
            s_max=self.s_max,
            s_min=self.s_min,
            rho=self.rho,
            final="zero",
        ).build(num_steps=n_step or self.n_step, device=device, dtype=dtype)
    ...
    def sample_coords(
        ...
        *,
        ...
        """Build the AF3 schedule and run the shared EDM integrator.
        ...
        plan = EDMRolloutPlan(
            schedule=schedule,
            coords_shape=(*batch_shape, N_sample, N_atom, 3),
            device=device,
            dtype=dtype,
            atom_mask=mask,
        )
        ...
        coords = self.runner.run(self.integrator, plan, predict, context).final_state
