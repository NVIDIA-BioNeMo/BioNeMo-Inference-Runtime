...
def test_protenix_distogram_head(sc: Scenario):
    ...
    ref = (
        RefProtenixDistogramHeadFromOSS.build(c_z=sc.c_z, no_bins=sc.no_bins)
    ...
    config = DistogramHeadConfig(c_z=sc.c_z, no_bins=sc.no_bins, dtype=sc.dtype)
    model = ProtenixDistogramHead(c_z=config.c_z, no_bins=config.no_bins, dtype=config.torch_dtype).to(device).eval()
    converted = convert_distogram_head_torch(config, ref.state_dict(), prefix="")
    missing, unexpected = model.load_state_dict(converted, strict=False)
    assert not missing and not unexpected, (missing, unexpected)

    z = torch.randn(sc.batch_size, sc.n_token, sc.n_token, sc.c_z, device=device)
