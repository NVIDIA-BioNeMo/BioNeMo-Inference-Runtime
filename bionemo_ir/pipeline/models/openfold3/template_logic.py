...
def resolve_template_idx_map(template: SelectedTemplate) -> np.ndarray | None:
    ...
    OSS compares two counts taken over the **aligned** subset only: ``repeats``,
    the query token starts whose ``res_id`` appears in ``idx_map[:, 0]``, against
    ``residue_starts`` of the template atom array subset to ``idx_map[:, 1]``. A
    template covering part of the query is therefore kept, contributing just its
    aligned residues; the counts diverge only when one template ``res_id``
    carries several altloc copies, which OSS resolves by keeping the
    highest-occupancy one before re-checking.

    Two regimes for a protein query (one token per residue):

    * **All-standard template** — kept iff the alignment is one-to-one between
      query residues and template positions. ``align_query_to_template_chain``
      emits mutually-ungapped pairs, and ``chain_data`` is keyed by position
      (altlocs already collapsed), so this holds unless the map is malformed.
    * **Template with a modified residue** — every polymer amino-acid residue
    ...
    if not nonstd:
        # OSS repeats (aligned query residues) vs residue_starts (aligned
        # template residues) — both counted over the alignment, not the chain.
        n_query_res = np.unique(idx_map[:, 0]).size
        n_template_res = np.unique(idx_map[:, 1]).size
        return idx_map if n_query_res == n_template_res else None
