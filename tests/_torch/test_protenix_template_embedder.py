...
from tensorrt_bionemo.models.protenix.convert import convert_template_embedder_torch
from tensorrt_bionemo.utils import str_dtype_to_torch
from tests._torch import skip_if_cutedsl
from tests.common.test_utils.protenix.ref_layers_from_oss import RefProtenixTemplateEmbedderFromOSS
...
def _make_features(sc: Scenario, device: torch.device) -> dict:
    ...
    asym = (torch.arange(N) // max(1, N // sc.n_chains)).clamp(max=sc.n_chains - 1).to(device)
    return {
        "asym_id": asym,  # [N]
        "template_distogram": rnd(T, N, N, 39),
        "template_pseudo_beta_mask": (rnd(T, N, N) > 0).float(),
        "template_aatype": torch.randint(0, 32, (T, N), generator=g).to(device),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        Scenario(dtype="float32", pairformer_dtype="bfloat16", tri_backend="CuTeDSL"),
    ],
    ids=["fp32", "bf16", "fp32-pairformer-bf16", "cutedsl"],
)
def test_protenix_template_embedder(sc: Scenario):
    ...
    ref = (
        RefProtenixTemplateEmbedderFromOSS.build(n_blocks=sc.n_blocks, c=sc.c, c_z=sc.c_z)
    ...
    converted = convert_template_embedder_torch(config, ref.state_dict(), prefix="")
    ...
    assert r < tol, f"rmse_ratio={r:.3e} exceeds {tol:.0e} (dtype={sc.dtype}, pairformer={sc.pairformer_dtype})"
...
def test_protenix_template_split_projection_matches_dense(dtype_name: str):
    ...
    sc = Scenario(n_token=12, n_templ=1, c=64, c_z=64, n_blocks=0, dtype=dtype_name, pairformer_dtype=dtype_name)
    config = TemplateEmbedderConfig(
        c=sc.c,
        c_z=sc.c_z,
        n_blocks=0,
    ...
    pair_mask = (torch.rand(1, sc.n_token, sc.n_token, device=device) > 0.2).to(dtype)
    multichain_mask = (batched["asym_id"][..., :, None] == batched["asym_id"][..., None, :]).to(dtype)
    masked_by = pair_mask * multichain_mask

    with torch.inference_mode():
        dgram = batched["template_distogram"][:, 0].to(dtype) * masked_by.unsqueeze(-1)
        pseudo_beta = (batched["template_pseudo_beta_mask"][:, 0].to(dtype) * masked_by).unsqueeze(-1)
        aatype = F.one_hot(batched["template_aatype"][:, 0], num_classes=32).to(dtype)
        aatype_i = aatype.unsqueeze(1).expand(-1, sc.n_token, -1, -1)
        aatype_j = aatype.unsqueeze(2).expand(-1, -1, sc.n_token, -1)
        unit_vector = batched["template_unit_vector"][:, 0].to(dtype) * masked_by.unsqueeze(-1)
        backbone = (batched["template_backbone_frame_mask"][:, 0].to(dtype) * masked_by).unsqueeze(-1)
        dense = torch.cat(
            [
                dgram,
                pseudo_beta,
    ...
    tol = {"atol": 2e-5, "rtol": 2e-5} if dtype == torch.float32 else {"atol": 5e-3, "rtol": 5e-2}
