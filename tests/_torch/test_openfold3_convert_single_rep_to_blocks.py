...
def test_query_to_keys(sc: Scenario):
    ...
    atom_mask = torch.randint(0, 2, (1, K, W), dtype=sc.torch_dtype, device=device)
    output = attn_metadata.query_to_keys(ref_pos.flatten(1, 2)).squeeze(1)
    output_atom_mask = atom_mask.unsqueeze(-1) * attn_metadata.query_to_keys(atom_mask.unsqueeze(-1)).squeeze(
        1
