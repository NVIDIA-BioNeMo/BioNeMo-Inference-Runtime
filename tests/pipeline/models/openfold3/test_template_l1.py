...
a distinct featurization case:
  ...
  8c4d  partial-coverage template retained by aligned-subset keep/drop
...
The golden is generated from the current implementation. Its template selection
matches the shipped P2 baseline (OpenFold3 0.4.3 at submodule revision
``0bb17be5``) with the recorded template-gap-row fix applied.
...
def _build_template_features(target: str) -> dict[str, torch.Tensor]:
    ...
    for p in doc["polymers"]:
        ...
        if p.get("templates"):
            ...
            templates = [
                TemplateParsed(
                    content=(FIXTURE_DIR / e["path"]).read_text(),
    ...
    trt = TemplateFeatureGenerator(config=None, name="t")({"token_index": sf["token_index"]}, ctx)
...
def _assert_matches_golden(sig: dict, gold: dict, target: str) -> None:
    exact = [
        ...
        "backbone_frame_mask",
        "distogram_bin_hist",
        "distogram_valid_pairs",
        "unit_vector_nonzero",
    ]
    for k in exact:
        assert torch.equal(sig[k], gold[k]), f"{target}: {k} mismatch vs golden"
    assert torch.allclose(sig["unit_vector_sum"], gold["unit_vector_sum"], atol=_UV_ATOL, rtol=0.0), (
...
def _regen_golden() -> None:
    ...
    for t in TARGETS:
        ...
        print(
            f"  {t}: n_tokens={int(golden[t]['n_tokens'])} "
