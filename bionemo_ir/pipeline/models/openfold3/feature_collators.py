...
class OpenFold3FinalFeatureCollator(FeatureCollatorBase):
    ...
    def __call__(
        ...
        # Preserve writer-only metadata after the feature stage replaces the
        # tokenizer row. Arbitrary CCD codes cannot be represented by the
        # model's fixed restype vocabulary, so the postprocessor needs this
        # sidecar to recover ligand identity.
        row = context.get("_row") or context
        structure = row.get("structure") if isinstance(row, dict) else None
        if isinstance(structure, dict):
            token_resnames = structure.get("token_resnames")
            token_mol_types = structure.get("token_mol_types")
            if token_resnames is not None:
                out["token_resnames"] = list(token_resnames)
            if token_mol_types is not None:
                out["token_mol_types"] = list(token_mol_types)
