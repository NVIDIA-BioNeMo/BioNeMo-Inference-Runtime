...
SEQ_FEATURES = ('residue_index', 'aatype', 'all_atom_positions',
                ...
                'prediction_atom_mask', 'literature_positions',
                'atom_indices_to_group_indices', 'rigid_group_default_frame')
    ...
    for chain_num, chain in enumerate(chains):
        ...
        for feature_name in chain_keys:
            if feature_name.endswith('_all_seq'):
                ...
                new_chain[feature_name] = feats_padded[paired_rows[:,
                                                                   chain_num]]
...
def _match_rows_by_sequence_similarity(
        this_species_msa_dfs: List[pd.DataFrame]) -> List[List[int]]:
    ...
    all_paired_msa_rows = np.array(all_paired_msa_rows).T.tolist()
    return all_paired_msa_rows


def pair_sequences(
    examples: List[Mapping[str, np.ndarray]], ) -> Dict[int, np.ndarray]:
    ...
    for chain_features in examples:
        ...
        common_species.update(species_dict)
    ...
    for species in common_species:
        ...
        if np.any(np.array(species_df_lengths) > 600):
            continue
...
def reorder_paired_rows(
    ...
    for num_pairings in sorted(all_paired_msa_rows_dict, reverse=True):
        ...
        paired_rows_product = np.abs(np.prod(paired_rows, axis=1))
...
def merge_chain_features(np_chains_list: List[Mapping[str, np.ndarray]],
    ...
    """
    np_chains_list = _pad_templates(np_chains_list,
...
def deduplicate_unpaired_sequences(
    np_chains: List[Mapping[str,
                            np.ndarray]]) -> List[Mapping[str, np.ndarray]]:
    ...
    for chain in np_chains:
        ...
        chain['num_alignments'] = np.array(chain['msa'].shape[0],
                                           dtype=np.int32)
    return np_chains
