...
from .feature_collators import (
    ...
    MakeMaskedMsa,
    MakeMsaFeat,
    MultimerCreateMsaFeat,
    MultimerMakeMaskedMsa,
...
class SampleRepeater(FeatureCollatorBase):
    ...
    def __init__(
        ...
        get_n_iters: Callable | None = None,
        stack_dim: int = -1,
        **kwargs,
    ):
        ...
        if get_n_iters is None:
            ...
            def get_n_iters(_: BaseConfig | None) -> int:
                return 1
    ...
    def __call__(self, batch: dict[str, torch.Tensor], context: dict[str, Any]) -> dict[str, torch.Tensor]:
        ...
        for k in keys:
            ret[k] = torch.stack([batch_i[k] for batch_i in ensemble_batch], dim=self.stack_dim)
        return ret
...
def create_ensemble_feature_collator() -> list[FeatureCollatorSpec]:
    feature_collator_specs = [
        ...
        FeatureCollatorSpec(name="make_masked_msa", functor=MakeMaskedMsa, kwargs={}),
        FeatureCollatorSpec(name="nearest_neighbor_clusters", functor=NearestNeighborClusters, kwargs={}),
        FeatureCollatorSpec(name="summarize_clusters", functor=SummarizeClusters, kwargs={}),
        FeatureCollatorSpec(name="crop_extra_msa", functor=CropExtraMsa, kwargs={}),
        FeatureCollatorSpec(name="delete_extra_msa", functor=DeleteExtraMsa, kwargs={}),
        FeatureCollatorSpec(name="make_msa_feat", functor=MakeMsaFeat, kwargs={}),
        FeatureCollatorSpec(name="select_feat", functor=SelectFeat, kwargs={"include_feats": _MONOMER_FEATURE_KEYS}),
        FeatureCollatorSpec(name="random_crop_to_size", functor=RandomCropToSize, kwargs={}),
...
class FeatureFactory(FeatureFactoryBase):
    ...
    feature_generator_specs: list[FeatureGeneratorSpec] = [
        ...
        FeatureGeneratorSpec(name="make_template_pseudo_beta", functor=MakeTemplatePseudoBeta, kwargs={}),
        FeatureGeneratorSpec(
            name="template_atom37_to_torsion_angles", functor=Atom37ToTorsionAngles, kwargs={"prefix": "template_"}
        ),
        FeatureGeneratorSpec(name="make_atom14_masks", functor=MakeAtom14Masks, kwargs={}),
    ]
...
def create_ensemble_multimer_feature_collator() -> list[FeatureCollatorSpec]:
    feature_collator_specs = [
        FeatureCollatorSpec(name="sample_msa", functor=MultimerSampleMsa, kwargs={}),
        FeatureCollatorSpec(name="make_masked_msa", functor=MultimerMakeMaskedMsa, kwargs={}),
        FeatureCollatorSpec(name="nearest_neighbor_clusters", functor=MultimerNearestNeighborClusters, kwargs={}),
        FeatureCollatorSpec(name="create_msa_feat", functor=MultimerCreateMsaFeat, kwargs={}),
        FeatureCollatorSpec(name="select_feat", functor=SelectFeat, kwargs={"include_feats": _MULTIMER_FEATURE_KEYS}),
        FeatureCollatorSpec(name="random_crop_to_size", functor=RandomCropToSize, kwargs={}),
        FeatureCollatorSpec(name="make_fixed_size", functor=MakeFixedSize, kwargs={}),
    ]
    return [
...
class MultimerFeatureFactory(FeatureFactoryBase):
    pre_init: Callable = pre_init
    feature_generator_specs: list[FeatureGeneratorSpec] = [
        FeatureGeneratorSpec(name="make_msa_profile", functor=MultimerMakeMsaProfile, kwargs={}),
        FeatureGeneratorSpec(name="create_target_features", functor=MultimerCreateTargetFeatures, kwargs={}),
        FeatureGeneratorSpec(name="make_atom14_masks", functor=MakeAtom14Masks, kwargs={}),
    ]
    features_merger_func: Callable = default_context_and_feature_merger
    feature_collator_specs: list[FeatureCollatorSpec] = create_ensemble_multimer_feature_collator()
