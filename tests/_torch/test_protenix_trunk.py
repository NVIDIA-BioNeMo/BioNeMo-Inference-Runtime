...
def _small_trunk_config(c_s: int,
                        c_z: int,
                        c_s_inputs: int,
                        pair_state_dtype: str = "float32") -> TrunkConfig:
    # Keep random-weight assembly stable in fp32.
    template = TemplateEmbedderConfig(c=32,
                                      c_z=c_z,
                                      n_blocks=1,
                                      pairwise_head_width=16,
                                      pairwise_num_heads=2,
                                      pairformer_dtype="float32")
    msa = ProtenixMSAModuleConfig(c_m=64,
                                  c_z=c_z,
                                  c_hidden_mul=c_z,
                                  ...
                                  dtype="float32")
    pairformer = PairformerConfig(token_s=c_s,
                                  token_z=c_z,
                                  num_blocks=2,
                                  num_heads=4,
...
def test_protenix_trunk_smoke():
    ...
    trunk = ProtenixTrunk(_small_trunk_config(c_s, c_z,
                                              c_s_inputs)).to(device).eval()

    feat = {
        "asym_id": (torch.arange(N) // 8).unsqueeze(0).to(device),
        "msa":
        torch.randint(0, 32, (B, S, N), device=device),
        "has_deletion": (torch.randn(B, S, N, device=device) > 0).float(),
        "deletion_value":
        torch.rand(B, S, N, device=device),
        "template_distogram":
        torch.randn(B, 1, N, N, 39, device=device),
        "template_pseudo_beta_mask": (torch.randn(B, 1, N, N, device=device)
                                      > 0).float(),
        "template_aatype":
        torch.randint(0, 32, (B, 1, N), device=device),
        "template_unit_vector":
        torch.randn(B, 1, N, N, 3, device=device),
        "template_backbone_frame_mask": (torch.randn(B, 1, N, N, device=device)
                                         > 0).float(),
    }
...
def test_protenix_bf16_pair_state_parity_and_input_safety():
    ...
    trunk_fp32 = ProtenixTrunk(
        _small_trunk_config(c_s, c_z, c_s_inputs,
                            pair_state_dtype="float32")).to(device).eval()
    ...
    trunk_bf16 = ProtenixTrunk(
        _small_trunk_config(c_s, c_z, c_s_inputs,
                            pair_state_dtype="bfloat16")).to(device).eval()
    ...
    with torch.inference_mode():
        s_fp32, z_fp32 = trunk_fp32({},
                                    s_inputs,
                                    s_init,
                                    z_init,
                                    num_cycles=10)
        s_bf16, z_bf16 = trunk_bf16({},
                                    s_inputs,
                                    s_init,
                                    z_init,
                                    num_cycles=10)
    ...
    def rmse_ratio(actual: torch.Tensor, expected: torch.Tensor) -> float:
        ...
        return (torch.mean((actual - expected)**2).sqrt() /
                (torch.mean(expected**2).sqrt() + 1e-8)).item()
