...
line ranges, and TRT-BioNeMo-specific policy is called out where it diverges.
"""
...
def stable_top_k(items: Sequence[_T], k: int,
                 key: Callable[[_T], float]) -> list[_T]:
    """Return the highest-scoring ``k`` items with stable input-order ties."""
    if k < 0:
        raise ValueError(f"k must be non-negative, got {k}")
    ranked = sorted(enumerate(items),
                    key=lambda indexed: (-float(key(indexed[1])), indexed[0]))
...
def align_query_to_template_chain(
    ...
    sequence_identity, query_coverage = seq_identity_and_coverage(
        query_aligned, template_aligned, query_seq)
    query_ids, template_ids = calculate_ids_hit(query_aligned,
                                                template_aligned)
    both_present = (query_ids != -1) & (template_ids != -1)
    idx_map = np.stack([query_ids[both_present], template_ids[both_present]],
                       axis=-1)
...
def _category_column(category, name: str, dtype: type) -> Optional[np.ndarray]:
...
def _entity_polymer_data(block) -> tuple[dict[str, str], dict[str, str]]:
    ...
    for index, entity_id in enumerate(entity_ids.tolist()):
        ...
        if sequences is not None:
            sequence_by_entity[entity_id] = _clean_canonical_sequence(
                str(sequences[index]))
...
def _modified_residue_parents(block) -> dict[str, str]:
    ...
    for residue_name, parent_name in zip(residue_names.tolist(),
                                         parent_names.tolist()):
        parent_name = str(parent_name).strip().upper()
        if parent_name not in {"", ".", "?"}:
            parents[str(residue_name).strip().upper()] = parent_name.split(
                ",", maxsplit=1)[0]
    return parents


def _residue_to_one_letter(residue_name: str,
                           modified_parents: Mapping[str, str]) -> str:
...
def _canonical_sequence_by_label(
    block, ) -> tuple[dict[str, str], dict[str, dict[int, str]], list[str]]:
    ...
    for index, (label, position, residue_name) in enumerate(
        ...
        names_by_label[label].setdefault(int(position),
    ...
    for label in label_order:
        ...
        inferred_is_protein = any(
            _residue_to_one_letter(name, modified_parents) != "X"
        ...
        if (len(entity_sequence) == max_position
                and all(residue.isalpha() for residue in entity_sequence)):
            canonical = entity_sequence
        else:
            canonical = "".join(
                _residue_to_one_letter(names.get(position, "UNK"),
                                       modified_parents)
...
def _center_atom_coordinates(
    ...
    coordinates = [
        ...
        for position in sorted(coords_by_pos) for atom_name in rc.atom_types
    ...
    return {
        position: {
            atom_name:
            (coordinate.astype(np.float32) - center).astype(np.float32)
...
def _correct_arg_atom_names(residue_name: str,
...
def extract_template_chains(content: str,
    ...
    canonical_by_label, names_by_label, label_order = (
        _canonical_sequence_by_label(block))
    author_by_label = _author_chain_by_label(block)

    atom_rows_by_label: dict[str, list[tuple[int, str, str, np.ndarray]]] = {}
    if "atom_site" in block:
        try:
            atom_array = get_structure(cif,
                                       model=1,
                                       use_author_fields=False,
                                       altloc="occupancy")
        except Exception as error:
            raise ValueError(
                "Could not read atom_site from template CIF") from error
        for index in range(atom_array.array_length()):
            ...
            if not np.all(np.isfinite(coordinate)):
                raise ValueError(
                    ...
                    f"{atom_name!r}")
            atom_rows_by_label.setdefault(label, []).append(
                (position, residue_name, atom_name, coordinate))
    ...
    for label in label_order:
        ...
        if author_chain_id in chains:
            raise ValueError(
                f"Author chain ID {author_chain_id!r} maps to multiple CIF chains"
            )
...
# TRT-BNM policy for identity-times-coverage auto-selection; OSS mechanics:
...
def select_template_for_cif(
    ...
    content: str,
    fmt: str = "cif",
    specified_chain_id: Optional[str] = None,
    ...
    if specified_chain_id is not None:
        if specified_chain_id not in chains:
            raise ValueError(
                f"Template author chain {specified_chain_id!r} was not found; "
                f"available chains: {sorted(chains)}")
        candidates = [(specified_chain_id, chains[specified_chain_id])]
    else:
        candidates = list(chains.items())

    selected: Optional[SelectedTemplate] = None
    for chain_id, chain_data in candidates:
        idx_map, sequence_identity, query_coverage = (
            align_query_to_template_chain(query_seq, chain_data))
    ...
    if selected is None:
        chain_description = (f"author chain {specified_chain_id!r}"
                             if specified_chain_id is not None else
                             "every protein chain")
        raise ValueError(
            f"Query has no mutually aligned residues with {chain_description}")

    if (specified_chain_id is None and selected.score < weak_match_threshold):
        logger.warning(
            "Automatically selected weak template match %s_%s "
            "(identity=%.3f, coverage=%.3f, score=%.3f)",
            selected.chain_data.block_name, selected.chain_id,
            selected.sequence_identity, selected.query_coverage,
            selected.score)
    return selected
...
def _template_content(template: Any) -> str:
    ...
    if not isinstance(content, str) or not content.strip():
        raise ValueError(
            "Each supplied template must contain materialized CIF content")
    return content
...
def _empty_template_feats(num_residues: int) -> dict[str, np.ndarray]:
    ...
    return {
        "template_aatype":
        np.zeros((0, num_residues, len(rc.restypes_with_x_and_gap)),
                 dtype=np.float32),
        "template_all_atom_positions":
        np.zeros((0, num_residues, rc.atom_type_num, 3), dtype=np.float32),
        "template_all_atom_mask":
        np.zeros((0, num_residues, rc.atom_type_num), dtype=np.float32),
        "template_sum_probs":
...
def _aligned_template_sequence(query_length: int,
                               selected: SelectedTemplate) -> str:
    ...
    for query_position, template_position in selected.idx_map:
        ...
        if not (0 <= query_index < query_length
                and 0 <= template_index < len(canonical)):
            continue
        residue = canonical[template_index].upper()
        aligned[
            query_index] = residue if residue in rc.HHBLITS_AA_TO_ID else "X"
    return "".join(aligned)
...
def _pack_selected_template(
    ...
    domain_name = (f"{selected.chain_data.block_name.lower()}_"
                   f"{selected.chain_id}").encode("utf-8")
    return (aatype, positions, mask, domain_name,
            aligned_sequence.encode("utf-8"))


# Adapted parity; direct-CIF orchestration with deterministic TRT-BNM top-k:
...
def build_template_feats(
    query_sequence: str,
    templates: Optional[Sequence[Any]],
    ...
    *,
    ...
    if max_templates < 0:
        raise ValueError(
            f"max_templates must be non-negative, got {max_templates}")
    ...
    if len(templates) > max_templates * _INPUT_TEMPLATE_WARN_FACTOR:
        ...
        logger.warning(
            ...
            len(templates), suffix, max_templates, max_templates)
    ...
    for template_index, template in enumerate(templates):
        try:
            ...
            if specified_chain_id is not None and not isinstance(
                    specified_chain_id, str):
            ...
            suffix = f" for query chain {chain_id!r}" if chain_id else ""
            raise ValueError(
                f"Invalid supplied template at index {template_index}{suffix}: "
                f"{error}") from error
        selected_templates.append(selected)

    selected_templates = stable_top_k(selected_templates,
                                      max_templates,
                                      key=lambda item: item.score)
    packed = [
        _pack_selected_template(query_sequence, selected)
        for selected in selected_templates
    ]
    return {
        "template_aatype":
        np.stack([item[0] for item in packed]).astype(np.int64),
        "template_all_atom_positions":
        np.stack([item[1] for item in packed]).astype(np.float32),
        "template_all_atom_mask":
        np.stack([item[2] for item in packed]).astype(np.float32),
        "template_sum_probs":
        np.ones((len(packed), 1), dtype=np.float32),
        "template_domain_names":
        np.asarray([item[3] for item in packed], dtype=object),
        "template_sequence":
        np.asarray([item[4] for item in packed], dtype=object),
    }
