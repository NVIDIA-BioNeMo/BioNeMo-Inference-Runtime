...
enforce automatically, whether it is authorized by developers or AI agents.
...
- **Local:** `uv sync --locked`, then `uv run --locked prek install` (the
  `commit-msg` shim runs the commit-message linter). Hooks then run on
  `git commit`.
...
| Documentation        | `fern/src/check.py` (hook `docs-check`, CI `docs:check`)         |
...
appears with what breaks if it moves. Do not align them by hand.
...
- Catch the narrowest exception set possible; keep the `try` body minimal and
  put logic in `else`. Prefer builtin exception types. Raise `ValueError`, do
  not `assert`, for invalid input.
...
- Reserve inline comments for non-obvious logic; do not restate the code.
...
- **`clang-format`** runs in the style gate (fast, no build). It is pinned to a
...
MRs on non-proprietary paths mirror to GitHub. A GitHub PR, after it is
approved, is imported as an internal MR, merged there, and synced back to
GitHub; the original PR is then closed, so a merged PR shows as closed rather
than merged.
...
- Allowed types: `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`,
  `build`, `ci`, `chore`, `revert`. A scope is allowed but not required;
  components are not enforced.
