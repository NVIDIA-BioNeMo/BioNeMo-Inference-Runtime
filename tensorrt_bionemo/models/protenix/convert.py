...
def _convert_atom_encoder_linears(out: dict, weights: dict, src_prefix: str,
                                  dst_prefix: str, dtype: torch.dtype) -> None:
    ...
    for target, sources in _FUSED_ATOM_LINEARS.items():
        out[_join(dst_prefix, f"{target}.weight")] = torch.cat(
            [
                weights[_join(src_prefix, f"linear_no_bias_{source}.weight")]
...
def _copy_to(out: dict,
             weights: dict,
             src: str,
             tgt: str,
             dtype: torch.dtype,
             *,
             unsqueeze: Optional[int] = None) -> None:
...
def _convert_adaln(out: dict, weights: dict, src: str, tgt: str,
    ...
    """
    out[f"{tgt}.s_norm.weight"] = weights[f"{src}.layernorm_s.weight"].to(
        dtype)
    out[f"{tgt}.fused_s_scale_s_bias.weight"] = torch.cat([
        weights[f"{src}.linear_s.weight"],
        weights[f"{src}.linear_nobias_s.weight"]
    ],
                                                          dim=0).to(dtype)
    s_bias = weights[f"{src}.linear_s.bias"]
    out[f"{tgt}.fused_s_scale_s_bias.bias"] = torch.cat(
        [s_bias, torch.zeros_like(s_bias)], dim=0).to(dtype)


def _convert_dit_attention(out: dict, weights: dict, src_apb: str,
                           tgt_layer: str, tgt_pba: str,
                           dtype: torch.dtype) -> None:
    """Shared DiT attention remap: fused KV ``[k; v]``, proj_z LN+Linear, gate."""
    attn = f"{src_apb}.attention"
    out[f"{tgt_pba}.proj_q.weight"] = weights[f"{attn}.linear_q.weight"].to(
        dtype)
    ...
    out[f"{tgt_pba}.proj_kv.weight"] = torch.cat([
        weights[f"{attn}.linear_k.weight"], weights[f"{attn}.linear_v.weight"]
    ],
                                                 dim=0).to(dtype)
    out[f"{tgt_pba}.proj_g.weight"] = weights[f"{attn}.linear_g.weight"].to(
        dtype)
    out[f"{tgt_pba}.proj_o.weight"] = weights[f"{attn}.linear_o.weight"].to(
        dtype)
    out[f"{tgt_pba}.proj_z.0.weight"] = weights[
        f"{src_apb}.layernorm_z.weight"].to(dtype)
    out[f"{tgt_pba}.proj_z.1.weight"] = weights[
        f"{src_apb}.linear_nobias_z.weight"].to(dtype)
    out[f"{tgt_layer}.output_projection.weight"] = weights[
        f"{src_apb}.linear_a_last.weight"].to(dtype)
    out[f"{tgt_layer}.output_projection.bias"] = weights[
        f"{src_apb}.linear_a_last.bias"].to(dtype)


def _convert_dit_transition(out: dict, weights: dict, src_blk: str,
                            tgt_layer: str, dtype: torch.dtype) -> None:
    ...
    out[f"{tgt_tr}.fused_swl_a_to_b.weight"] = torch.cat([
        weights[f"{src_ct}.linear_nobias_a2.weight"],
        weights[f"{src_ct}.linear_nobias_a1.weight"]
    ],
                                                         dim=0).to(dtype)
    out[f"{tgt_tr}.b_to_a.weight"] = weights[
        f"{src_ct}.linear_nobias_b.weight"].to(dtype)
    out[f"{tgt_tr}.output_projection.weight"] = weights[
        f"{src_ct}.linear_s.weight"].to(dtype)
    out[f"{tgt_tr}.output_projection.bias"] = weights[
        f"{src_ct}.linear_s.bias"].to(dtype)


def _convert_protenix_atom_dit_block(out: dict, weights: dict, src_blk: str,
                                     tgt_layer: str,
                                     dtype: torch.dtype) -> None:
    ...
    _convert_adaln(out, weights, f"{src_apb}.layernorm_a",
                   f"{tgt_pba}.layer_norm_a_q", dtype)
    _convert_adaln(out, weights, f"{src_apb}.layernorm_kv",
                   f"{tgt_pba}.layer_norm_a_k", dtype)
...
def _convert_protenix_token_dit_block(out: dict, weights: dict, src_blk: str,
                                      tgt_layer: str,
                                      dtype: torch.dtype) -> None:
...
def convert_hf_input_embedder_torch(config: BaseConfig,
    ...
    for i in (1, 3, 5):
        out[f"{ae}.small_mlp.{i}.weight"] = weights[src_ae(
            f"small_mlp.{i}.weight")].to(dtype)
    ...
    for i in range(num_blocks):
        _convert_protenix_atom_dit_block(
            ...
            f"{ae}.atom_transformer.layers.{i}", dtype)

    if config.esm_enabled:
        out["linear_esm.weight"] = weights[_join(
            prefix, "linear_esm.weight")].to(dtype)
...
def convert_relative_position_encoding_torch(
        config: BaseConfig,
        weights: dict,
        prefix: str = "relative_position_encoding") -> dict:
    ...
    dtype = config.torch_dtype
    return {
        "linear.weight": weights[_join(prefix,
                                       "linear_no_bias.weight")].to(dtype)
    }


def convert_constraint_embedder_torch(
        config: BaseConfig,
        weights: dict,
    ...
    for is_enabled, name in enabled:
        if is_enabled:
            out[f"{name}.weight"] = weights[_join(prefix,
                                                  f"{name}.weight")].to(dtype)
    return out


def _openfold_pairformer_block_intermediate(weights: dict, src_blk: str,
                                            dst_blk: str, out: dict) -> None:
    ...
    for mul in ("tri_mul_out", "tri_mul_in"):
        ...
        out[f"{d}.p_in.weight"] = torch.cat([
            weights[f"{s}.linear_a_p.weight"],
            weights[f"{s}.linear_b_p.weight"]
        ],
                                            dim=0)
        out[f"{d}.g_in.weight"] = torch.cat([
            weights[f"{s}.linear_a_g.weight"],
            weights[f"{s}.linear_b_g.weight"]
        ],
                                            dim=0)
    ...
    for att in ("tri_att_start", "tri_att_end"):
        ...
        for p in ("q", "k", "v", "o", "g"):
            out[f"{d}.mha.linear_{p}.weight"] = weights[
                f"{s}.mha.linear_{p}.weight"]
...
def _convert_pair_path(weights: dict,
                       src_pair: str,
                       tgt_layer: str,
                       out: dict,
                       dtype_str: str,
...
def convert_template_embedder_torch(config: BaseConfig,
    ...
    for ln in ("layernorm_z", "layernorm_v"):
        _copy_to(out, weights, _join(prefix, f"{ln}.weight"), f"{ln}.weight",
                 dtype)
        _copy_to(out, weights, _join(prefix, f"{ln}.bias"), f"{ln}.bias",
                 dtype)
    for lin in ("linear_no_bias_z", "linear_no_bias_a", "linear_no_bias_u"):
        _copy_to(out, weights, _join(prefix, f"{lin}.weight"), f"{lin}.weight",
                 dtype)

    for i in range(config.n_blocks):
        _convert_pair_path(weights,
...
def _convert_pairformer_single_path(weights: dict, src_blk: str,
                                    tgt_layer: str, out: dict,
                                    dtype: torch.dtype) -> None:
    ...
    out[f"{a}.proj_z.0.weight"] = weights[f"{apb}.layernorm_z.weight"].to(
        dtype)
    out[f"{a}.proj_z.0.bias"] = weights[f"{apb}.layernorm_z.bias"].to(dtype)
    out[f"{a}.proj_z.1.weight"] = weights[f"{apb}.linear_nobias_z.weight"].to(
        dtype)
    ...
    out[f"{t}.fused_fc2_fc1.weight"] = torch.cat([
        weights[f"{st}.linear_no_bias_b.weight"],
        weights[f"{st}.linear_no_bias_a.weight"]
    ],
                                                 dim=0).to(dtype)
    out[f"{t}.fc3.weight"] = weights[f"{st}.linear_no_bias.weight"].to(dtype)


def convert_pairformer_stack_torch(config: BaseConfig,
                                   weights: dict,
                                   prefix: str = "pairformer_stack") -> dict:
...
def _convert_opm(weights: dict, src: str, tgt: str, out: dict,
    ...
    out[f"{tgt}.fused_proj_a_b.weight"] = torch.cat(
        [weights[f"{src}.linear_1.weight"], weights[f"{src}.linear_2.weight"]],
        dim=0).to(dtype)
...
def _convert_msa_pwa(weights: dict, src: str, tgt: str, out: dict,
    ...
    out[f"{tgt}.norm_z.weight"] = weights[f"{src}.layernorm_z.weight"].to(
        dtype)
    out[f"{tgt}.norm_z.bias"] = weights[f"{src}.layernorm_z.bias"].to(dtype)
    out[f"{tgt}.fused_proj_m_g.weight"] = torch.cat([
        weights[f"{src}.linear_no_bias_mv.weight"],
        weights[f"{src}.linear_no_bias_mg.weight"]
    ],
                                                    dim=0).to(dtype)
    out[f"{tgt}.proj_z.weight"] = weights[f"{src}.linear_no_bias_z.weight"].to(
        dtype)
    out[f"{tgt}.proj_o.weight"] = weights[
        f"{src}.linear_no_bias_out.weight"].to(dtype)
...
def _convert_swiglu_transition(weights: dict, src: str, tgt: str, out: dict,
    ...
    out[f"{tgt}.fused_fc2_fc1.weight"] = torch.cat([
        weights[f"{src}.linear_no_bias_b.weight"],
        weights[f"{src}.linear_no_bias_a.weight"]
    ],
                                                   dim=0).to(dtype)
    out[f"{tgt}.fc3.weight"] = weights[f"{src}.linear_no_bias.weight"].to(
        dtype)


def convert_msa_module_torch(config: BaseConfig,
                             weights: dict,
                             prefix: str = "msa_module") -> dict:
    ...
    for lin in ("linear_no_bias_m", "linear_no_bias_s"):
        out[f"{lin}.weight"] = weights[_join(prefix,
                                             f"{lin}.weight")].to(dtype)

    for i in range(config.no_blocks):
        src_blk = _join(prefix, f"blocks.{i}")
        tgt = f"msa_stack.blocks.{i}"
        _convert_opm(weights, f"{src_blk}.outer_product_mean_msa",
                     f"{tgt}.outer_product_mean", out, dtype)
        if i != config.no_blocks - 1:  # non-last block keeps the MSA sublayers
            _convert_msa_pwa(
                weights, f"{src_blk}.msa_stack.msa_pair_weighted_averaging",
                f"{tgt}.msa_att_row", out, dtype)
            _convert_swiglu_transition(weights,
                                       f"{src_blk}.msa_stack.transition_m",
                                       f"{tgt}.msa_transition", out, dtype)
        _convert_pair_path(weights,
...
def convert_diffusion_conditioning_torch(
        config: BaseConfig,
        weights: dict,
        prefix: str = "diffusion_module.diffusion_conditioning") -> dict:
    ...
    out["relpe.linear.weight"] = weights[_join(
        prefix, "relpe.linear_no_bias.weight")].to(dtype)
    ...
    out["fourier_embedding.proj.weight"] = weights[_join(
        prefix, "fourier_embedding.w")].to(dtype).unsqueeze(-1)
    out["fourier_embedding.proj.bias"] = weights[_join(
        prefix, "fourier_embedding.b")].to(dtype)

    # Pair transitions run in the cached ``z_pair_dtype`` (bf16); the
    # noise-dependent single transitions stay at the module dtype (fp32).
    for src, tgt in (("transition_z1", "transition_z.0"), ("transition_z2",
                                                           "transition_z.1")):
        _convert_swiglu_transition(weights, _join(prefix, src), tgt, out,
                                   z_pair_dtype)
    for src, tgt in (("transition_s1", "transition_s.0"), ("transition_s2",
                                                           "transition_s.1")):
        _convert_swiglu_transition(weights, _join(prefix, src), tgt, out,
                                   dtype)
    return out
...
def convert_diffusion_atom_encoder_torch(
        config: BaseConfig,
        weights: dict,
        prefix: str = "diffusion_module.atom_attention_encoder") -> dict:
    ...
    for i in (1, 3, 5):
        out[f"small_mlp.{i}.weight"] = weights[_join(
            prefix, f"small_mlp.{i}.weight")].to(dtype)
    ...
    for i in range(config.atom_transformer_config.num_blocks):
        _convert_protenix_atom_dit_block(
            out, weights,
            _join(prefix,
                  f"atom_transformer.diffusion_transformer.blocks.{i}"),
            f"atom_transformer.layers.{i}", dtype)
    return out
...
def convert_atom_attention_decoder_torch(
        config: BaseConfig,
        weights: dict,
        prefix: str = "diffusion_module.atom_attention_decoder") -> dict:
    ...
    for name in ("linear_no_bias_a", "linear_no_bias_out"):
        out[f"{name}.weight"] = weights[_join(prefix,
                                              f"{name}.weight")].to(dtype)
    out["layernorm_q.weight"] = weights[_join(prefix,
                                              "layernorm_q.weight")].to(dtype)
...
def convert_diffusion_module_torch(config: BaseConfig,
                                   weights: dict,
                                   prefix: str = "diffusion_module") -> dict:
    ...
    _copy_to(out, weights, _join(prefix, "layernorm_s.weight"),
             "layernorm_s.weight", dtype)
    _copy_to(out, weights, _join(prefix, "layernorm_a.weight"),
             "layernorm_a.weight", dtype)
    _copy_to(out, weights, _join(prefix, "linear_no_bias_s.weight"),
             "linear_no_bias_s.weight", dtype)

    _merge_prefix(
        out, "diffusion_conditioning",
        convert_diffusion_conditioning_torch(
            config.diffusion_conditioning_config,
            weights,
            prefix=_join(prefix, "diffusion_conditioning")))
    _merge_prefix(
        out, "atom_attention_encoder",
        convert_diffusion_atom_encoder_torch(config.atom_encoder_config,
                                             weights,
                                             prefix=_join(
                                                 prefix,
    ...
    _merge_prefix(
        ...
        convert_atom_attention_decoder_torch(config.atom_decoder_config,
                                             weights,
                                             prefix=_join(
                                                 prefix,
    ...
    for i in range(config.token_transformer_config.num_blocks):
        _convert_protenix_token_dit_block(out, weights,
                                          f"{tt_prefix}.blocks.{i}",
                                          f"diffusion_transformer.layers.{i}",
                                          dtype)
    return out


def convert_input_projections_torch(config: BaseConfig,
                                    weights: dict,
                                    prefix: str = "") -> dict:
    """Model-level s/z init projections (AF3 Alg. 1); identical OSS names."""
    dtype = config.torch_dtype
    names = ("linear_no_bias_sinit", "linear_no_bias_zinit1",
             "linear_no_bias_zinit2", "linear_no_bias_token_bond")
    return {
        name: weights[_join(prefix, f"{name}.weight")].to(dtype)
        for name in names
    }


def convert_trunk_torch(config: BaseConfig,
                        weights: dict,
                        prefix: str = "") -> dict:
    ...
    _merge_prefix(
        out, "template_embedder",
        convert_template_embedder_torch(config.template_embedder_config,
                                        weights,
                                        prefix=_join(prefix,
                                                     "template_embedder")))
    _merge_prefix(
        out, "msa_module",
        convert_msa_module_torch(config.msa_module_config,
                                 weights,
                                 prefix=_join(prefix, "msa_module")))
    _merge_prefix(
        out, "pairformer_stack",
        convert_pairformer_stack_torch(config.pairformer_config,
                                       weights,
                                       prefix=_join(prefix,
                                                    "pairformer_stack")))
    return out
...
def convert_distogram_head_torch(config: BaseConfig,
                                 weights: dict,
                                 prefix: str = "distogram_head") -> dict:
...
def convert_confidence_head_torch(config: BaseConfig,
                                  weights: dict,
                                  prefix: str = "confidence_head") -> dict:
    """Confidence head projections/norms + pairformer sub-conversion."""
    dtype = config.torch_dtype
    out: dict[str, torch.Tensor] = {}
    for lin in ("linear_no_bias_s1", "linear_no_bias_s2", "linear_no_bias_d",
                "linear_no_bias_d_wo_onehot", "linear_no_bias_pae",
                "linear_no_bias_pde"):
        _copy_to(out, weights, _join(prefix, f"{lin}.weight"), f"{lin}.weight",
                 dtype)
    for param in ("plddt_weight", "resolved_weight"):
        _copy_to(out, weights, _join(prefix, param), param, dtype)
    for ln in ("input_strunk_ln", "pae_ln", "pde_ln", "plddt_ln",
               "resolved_ln"):
        _copy_to(out, weights, _join(prefix, f"{ln}.weight"), f"{ln}.weight",
                 dtype)
        _copy_to(out, weights, _join(prefix, f"{ln}.bias"), f"{ln}.bias",
