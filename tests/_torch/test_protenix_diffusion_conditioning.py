...
from bionemo_ir._torch.utils import CHUNK_REGISTRY, DIFFUSION_PAIR_TRANSITION, ChunkPolicy
...
def test_protenix_diffusion_conditioning(sc: Scenario):
    ...
    ref = (
        RefProtenixDiffusionConditioningFromOSS.build(
            c_z=sc.c_z, c_s=sc.c_s, c_s_inputs=sc.c_s_inputs, c_noise_embedding=sc.c_noise
        )
    ...
    converted = convert_diffusion_conditioning_torch(config, ref.state_dict(), prefix="")
    ...
    assert all(p.dtype == z_pair_dtype for layer in model.transition_z for p in layer.parameters())
    assert all(p.dtype == torch_dtype for layer in model.transition_s for p in layer.parameters())
    assert all(layer.auto_chunk_policy is CHUNK_REGISTRY.get(DIFFUSION_PAIR_TRANSITION) for layer in model.transition_z)
...
def test_protenix_diffusion_pair_projection_without_concat(z_pair_dtype: str):
    ...
    relp = torch.randn(1, n, n, model.relpe.linear.in_features, dtype=torch.float32, device=device)
    z_trunk = torch.randn(1, n, n, c_z, dtype=torch.float32, device=device)

    with torch.inference_mode():
        relpe_z = model.relpe(relp=relp)
        expected = model.linear_no_bias_z(model.layernorm_z(torch.cat([z_trunk, relpe_z], dim=-1))).to(
            str_dtype_to_torch(z_pair_dtype)
        )
        actual = model._joint_layernorm_linear_z(z_trunk, relpe_z).to(str_dtype_to_torch(z_pair_dtype))

    tol = {"atol": 5e-5, "rtol": 5e-5} if z_pair_dtype == "float32" else {"atol": 2e-2, "rtol": 2e-2}
