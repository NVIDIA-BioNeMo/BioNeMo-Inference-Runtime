...
from test_utils.boltz.create_and_load_weights import (
    create_single_conditioning_weights,
    load_single_conditioning_weights_torch,
)
...
@pytest.mark.parametrize("sc", [Scenario(dtype="float32"), Scenario(dtype="float16", seq_len=1024)])
def test_single_conditioning(sc: Scenario):
    ...
    model = SingleConditioning(token_s=token_s, dim_fourier=dim_fourier, num_transitions=num_transitions).to(device)
    ...
    with torch.inference_mode():
        ...
        output, _ = model(times, s_trunk, s_inputs)  # squeeze out multiplicity dimension
