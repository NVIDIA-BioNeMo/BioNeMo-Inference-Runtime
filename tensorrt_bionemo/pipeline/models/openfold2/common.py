...
def shaped_categorical(probs: torch.Tensor,
...
def unsorted_segment_sum(data: torch.Tensor, segment_ids: torch.Tensor,
    ...
    assert (len(segment_ids.shape) == 1
            and segment_ids.shape[0] == data.shape[0])
...
def atom37_to_torsion_angles(
    ...
    pre_omega_mask = torch.prod(prev_all_atom_mask[..., 1:3],
                                dim=-1) * torch.prod(all_atom_mask[..., :2],
                                                     dim=-1)
    ...
    chi_atom_indices = torch.as_tensor(get_chi_atom_indices(),
    ...
    chi_angle_atoms_mask = torch.prod(chi_angle_atoms_mask,
                                      dim=-1,
