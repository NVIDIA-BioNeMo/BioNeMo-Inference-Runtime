  ...
  bool rms_norm)
{
  ...
  for (std::size_t index = 0; index < registry.count; ++index)
  {
    ...
    if (
      ...
      && image.num_threads == num_threads && image.is_rms_norm == rms_norm)
    {
      return image;
    }
  }
  ...
  throw std::invalid_argument(
    ...
    + ", num_threads=" + std::to_string(num_threads) + ", rms_norm=" + std::to_string(rms_norm)
    + ". N and norm type are compiled into the kernel, so an unshipped variant has no payload.");
}
  ...
  bool rms_norm)
{
  embedded::CubinImage const& image
    = find_embedded_cubin(target_sm, dtype, feature_dim, threads_per_row, num_threads, rms_norm);
