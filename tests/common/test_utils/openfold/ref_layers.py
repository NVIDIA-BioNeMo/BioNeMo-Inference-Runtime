...
class RefMSAAttention(nn.Module):
    def __init__(
        ...
        if not using_tri_attn:
            self.mha = RefPairwiseSelfAttention(
                ...
                inf=inf,
                bias_flags={"q": False, "k": False, "v": False, "g": True, "o": True},  # default for boltz
    ...
    def forward(self, m: torch.Tensor, z: torch.Tensor | None = None, mask: torch.Tensor | None = None):
        """
        Args:
    ...
    def load_weights(
        ...
        weights_biases_path = [(f"{layer_path}.layer_norm_m.weight", f"{layer_path}.layer_norm_m.bias")]
        ...
        if f"{layer_path}.layer_norm_z.weight" in state_dict:
            ...
            weights_biases_path.extend(
                [
                    (f"{layer_path}.layer_norm_z.weight", f"{layer_path}.layer_norm_z.bias"),
            ...
            num_heads = 8
        if using_tri_attn:
            mha = RefTriangleAttention.load_weights(model=model, layer_path=f"{layer_path}.mha", num_heads=num_heads)
            c_in = mha.c_q
            c_hidden = mha.c_hidden
            no_heads = mha.no_heads
        else:
            mha = RefPairwiseSelfAttention.load_weights(
                model=model, layer_path=f"{layer_path}.mha", num_heads=num_heads
            )
            c_in = mha.c_s
...
class RefOuterProductMean(BoltzRefOuterProductMean):
    ...
    def load_weights(
        ...
        weights_biases_path = [
            ...
            (f"{layer_path}.linear_out.weight", f"{layer_path}.linear_out.bias"),
        ]
...
class RefTriangleAttentionNode(BoltzRefTriangleAttentionNode):
    ...
    def load_weights(
        ...
        mha = RefTriangleAttention.load_weights(
            model=model, layer_path=f"{layer_path}.mha", state_dict=state_dict, num_heads=num_heads
        )
        c_in = mha.c_q
        c_hidden = mha.c_hidden

        m = cls(c_in=c_in, c_hidden=c_hidden, num_heads=num_heads, mha_bias_flags=mha.bias_flags, starting=starting)
...
class RefTriangleMultiplicationNode(BoltzRefTriangleMultiplicationNode):
    ...
    def load_weights(
        ...
        weights_biases_path = [
            (f"{layer_path}.layer_norm_in.weight", f"{layer_path}.layer_norm_in.bias"),
            (f"{layer_path}.layer_norm_out.weight", f"{layer_path}.layer_norm_out.bias"),
...
class RefEvoformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.msa_att_row = RefMSAAttention(
            c_in=c_m,
        ...
        self.tri_mul_out = RefTriangleMultiplicationNode(
            dim=c_z,
            outgoing=True,
            bias_flags={
                "p_out": True,
        ...
        self.tri_mul_in = RefTriangleMultiplicationNode(
            dim=c_z,
            outgoing=False,
            bias_flags={
                "p_in": True,
        ...
        if not self.no_column_attention:
            self.msa_att_col = RefMSAAttention(
                c_in=c_m,
                c_hidden=c_hidden_msa_att,
                no_heads=no_heads_msa,
                pair_bias=False,
    ...
    @classmethod
    def load_weights(
        cls, model: str = "openfold2_ptm_1", layer_path: str = "evoformer.blocks.0", state_dict: dict = None
    ):
        if state_dict is None:
            state_dict = load_weights(model, local_files_only=False)
        ...
        msa_att_col = RefMSAAttention.load_weights(
            model=model, layer_path=f"{layer_path}.msa_att_col", state_dict=state_dict
        )
        msa_transition = RefMSATransition.load_weights(
            model=model, layer_path=f"{layer_path}.core.msa_transition", state_dict=state_dict
        )
        outer_product_mean = RefOuterProductMean.load_weights(
            model=model, layer_path=f"{layer_path}.core.outer_product_mean", state_dict=state_dict
        )
        tri_mul_out = RefTriangleMultiplicationNode.load_weights(
            model=model, layer_path=f"{layer_path}.core.tri_mul_out", state_dict=state_dict
        )
        tri_mul_in = RefTriangleMultiplicationNode.load_weights(
            model=model, layer_path=f"{layer_path}.core.tri_mul_in", state_dict=state_dict
        )
        tri_attn_start = RefTriangleAttentionNode.load_weights(
            model=model, layer_path=f"{layer_path}.core.tri_att_start", state_dict=state_dict
        )
        tri_attn_end = RefTriangleAttentionNode.load_weights(
            model=model, layer_path=f"{layer_path}.core.tri_att_end", state_dict=state_dict
        )
        pair_transition = RefPairTransition.load_weights(
            model=model, layer_path=f"{layer_path}.core.pair_transition", state_dict=state_dict
        )
        m = cls(
        ...
        m.msa_att_row = msa_att_row
        m.msa_att_col = msa_att_col
        m.msa_transition = msa_transition
        m.outer_product_mean = outer_product_mean
        m.tri_mul_out = tri_mul_out
        m.tri_mul_in = tri_mul_in
        m.tri_attn_start = tri_attn_start
        m.tri_attn_end = tri_attn_end
        m.pair_transition = pair_transition
        return m

    def _compute_opm(self, m: torch.Tensor, z: torch.Tensor, msa_mask: torch.Tensor) -> torch.Tensor:
        opm = self.outer_product_mean(m, msa_mask)
        z = z + opm
        return m, z

    def forward(
        self, m: torch.Tensor, z: torch.Tensor, msa_mask: torch.Tensor, pair_mask: torch.Tensor
...
class RefMSAColumnGlobalAttention(nn.Module):
    ...
    def load_weights(
        ...
        global_attention = RefGlobalAttention.load_weights(
            model=model, layer_path=f"{layer_path}.global_attention", state_dict=state_dict
        )
        m = cls(c_in=global_attention.c_in, c_hidden=global_attention.c_hidden, no_heads=global_attention.no_heads)
        m.global_attention = global_attention

        m.layer_norm_m.weight.data.copy_(state_dict[f"{layer_path}.layer_norm_m.weight"])
        m.layer_norm_m.bias.data.copy_(state_dict[f"{layer_path}.layer_norm_m.bias"])
        return m
...
class RefExtraMSABlock(RefEvoformerBlock):
    def __init__(
        ...
        self.msa_att_col = RefMSAColumnGlobalAttention(
            c_in=c_m, c_hidden=c_hidden_msa_att, no_heads=no_heads_msa, inf=inf, eps=eps
        )

    def forward(self, m: torch.Tensor, z: torch.Tensor, msa_mask: torch.Tensor, pair_mask: torch.Tensor):
        if self.opm_first:
            m, z = self._compute_opm(m, z, msa_mask)
        m = m + self.msa_att_row(m, z=z, mask=msa_mask)
    ...
    @classmethod
    def load_weights(
        cls, model: str = "openfold2_ptm_1", layer_path: str = "extra_msa_stack.blocks.0", state_dict: dict = None
    ):
        if state_dict is None:
            state_dict = load_weights(model, local_files_only=False)
        msa_att_row = RefMSAAttention.load_weights(
...
class RefInputEmbedder(nn.Module):
    ...
    @classmethod
    def load_weights(cls, model: str = "openfold2_ptm_1", layer_path: str = "input_embedder", state_dict: dict = None):
        if state_dict is None:
            state_dict = load_weights(model, local_files_only=False)

        weights_biases_path = [
            (f"{layer_path}.linear_tf_z_i.weight", f"{layer_path}.linear_tf_z_i.bias"),
            (f"{layer_path}.linear_tf_z_j.weight", f"{layer_path}.linear_tf_z_j.bias"),
            (f"{layer_path}.linear_tf_m.weight", f"{layer_path}.linear_tf_m.bias"),
            (f"{layer_path}.linear_msa_m.weight", f"{layer_path}.linear_msa_m.bias"),
            (f"{layer_path}.linear_relpos.weight", f"{layer_path}.linear_relpos.bias"),
        ]
        ...
        m = cls(tf_dim=tf_dim, msa_dim=msa_dim, c_z=c_z, c_m=c_m, relpos_k=relpos_k)
        layers = [m.linear_tf_z_i, m.linear_tf_z_j, m.linear_tf_m, m.linear_msa_m, m.linear_relpos]
        for (weights_path, bias_path), layer in zip(weights_biases_path, layers):
            if bias_path is not None:
                layer.bias.data.copy_(state_dict[bias_path])
            layer.weight.data.copy_(state_dict[weights_path])
...
class RefInputEmbedderMultimer(nn.Module):
    ...
    @classmethod
    def load_weights(
        cls, model: str = "alphafold2_multimer_1", layer_path: str = "input_embedder", state_dict: dict = None
    ):
        if state_dict is None:
            state_dict = load_weights(model, local_files_only=False)
...
class RefRecyclingEmbedder(nn.Module):
    ...
    @classmethod
    def load_weights(
        cls, model: str = "openfold2_ptm_1", layer_path: str = "recycling_embedder", state_dict: dict = None
    ):
        if state_dict is None:
            state_dict = load_weights(model, local_files_only=False)
        ...
        weights_biases_path = [
            ...
            (f"{layer_path}.layer_norm_m.weight", f"{layer_path}.layer_norm_m.bias"),
            (f"{layer_path}.layer_norm_z.weight", f"{layer_path}.layer_norm_z.bias"),
        ]
        ...
        m = cls(c_m=c_m, c_z=c_z, min_bin=min_bin, max_bin=max_bin, no_bins=no_bins, inf=inf)
...
class RefTemplatePairStackBlock(nn.Module):
    ...
    @classmethod
    def load_weights(
        cls, model: str = "openfold2_ptm_1", layer_path: str = "template_pair_stack.blocks.0", state_dict: dict = None
    ):
        if state_dict is None:
            state_dict = load_weights(model, local_files_only=False)

        tri_mul_out = RefTriangleMultiplicationNode.load_weights(
            model=model, layer_path=f"{layer_path}.tri_mul_out", state_dict=state_dict
        )
        tri_mul_in = RefTriangleMultiplicationNode.load_weights(
            model=model, layer_path=f"{layer_path}.tri_mul_in", state_dict=state_dict
        )
        tri_attn_start = RefTriangleAttentionNode.load_weights(
            model=model, layer_path=f"{layer_path}.tri_att_start", state_dict=state_dict
        )
        tri_attn_end = RefTriangleAttentionNode.load_weights(
            model=model, layer_path=f"{layer_path}.tri_att_end", state_dict=state_dict
        )
        pair_transition = RefPairTransition.load_weights(
...
class RefTemplatePointwiseAttention(nn.Module):
    def __init__(self, c_t, c_z, c_hidden, no_heads, inf):
        ...
        self.mha = RefTriangleAttention(
            ...
            c_hidden=c_hidden,
            no_heads=self.no_heads,
            bias_flags={"q": False, "k": False, "v": False, "g": False, "o": True},
        )
