...
def metadata_cache_dir() -> Path:
    """Root scanned for staged metadata assets: ``BIOIR_METADATA``
    or ``<CACHE_DIR>/metadata``, where pre-downloaded assets may be placed to
    avoid a network fetch."""
    override = os.getenv("BIOIR_METADATA")
    if override:
        return Path(override)
    import bionemo_ir

    return bionemo_ir.CACHE_DIR / "metadata"
...
def get_model_cache_dir(
    ...
    if cache_dir is not None:
        ...
        d = bionemo_ir.CACHE_DIR / model_name
...
def load_metadata(
    ...
    for meta_file in HF_MODEL_METADATA[model_name]:
        ...
        # Fall back to an asset staged in the local cache before giving up
