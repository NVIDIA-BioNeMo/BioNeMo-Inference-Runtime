...
from tensorrt_bionemo.models.boltz1.convert import convert_hf_pairformer_torch as boltz1_convert_hf_pairformer_torch
from tensorrt_bionemo.models.boltz1.convert import (
...
def get_post_pre_norm_weights(
    state_dict: dict, prefix: str, tbm_prefix: str, post_layer_norm: bool = False, dtype: str = "float32"
):
    ...
    if post_layer_norm:
        ...
        ret[f"{tbm_prefix}.post_norm_s.weight"] = post_norm_weight.to(torch_dtype)
    ...
    for i in range(config.num_blocks):
        ...
        weights.update(
            get_post_pre_norm_weights(
                state_dict, f"{layer_prefix}", f"{layer_tbm_prefix}", config.post_layer_norm, dtype="float32"
            )
        )
        weights.update(
            get_pairwise_attn_weights(
...
def get_pairwise_conditioner_weights(
    state_dict: dict, prefix: str, tbm_prefix: str, num_transitions: int, dtype: str = "float32"
):
    torch_dtype = str_dtype_to_torch(dtype)
    ret = {}
    ...
    ret.update(
        {
            f"{tbm_prefix}.init_proj_norm.weight": init_proj_norm_weight.to(torch_dtype),
            f"{tbm_prefix}.init_proj_norm.bias": init_proj_norm_bias.to(torch_dtype),
            f"{tbm_prefix}.init_proj_linear.weight": init_proj_linear_weight.to(torch_dtype),
        }
    )
...
def get_pairformer_no_seq_weights(
    state_dict: dict, prefix: str, tbm_prefix: str, token_z: int = 128, dtype: str = "float32"
):
    str_dtype_to_torch(dtype)
    weights = {}
    weights.update(
        get_tri_attn_node_weights(state_dict, f"{prefix}.tri_att_start", f"{tbm_prefix}.tri_attn_start", dtype=dtype)
    )
    weights.update(
        get_tri_attn_node_weights(state_dict, f"{prefix}.tri_att_end", f"{tbm_prefix}.tri_attn_end", dtype=dtype)
    )
    weights.update(
        get_tri_mul_node_weights(state_dict, f"{prefix}.tri_mul_out", f"{tbm_prefix}.tri_mul_out", dtype=dtype)
    )
    weights.update(
        get_tri_mul_node_weights(state_dict, f"{prefix}.tri_mul_in", f"{tbm_prefix}.tri_mul_in", dtype=dtype)
    )

    weights.update(
        get_transition_weights(
            state_dict, f"{prefix}.transition_z", f"{tbm_prefix}.transition_z", token_z * 4, dtype=dtype
        )
    )
    return weights
...
def convert_hf_affinity_module_torch(
    ...
    **kwargs,
    ...
    tbnm_state_dict = {}
    tbnm_state_dict["dist_bin_pairwise_embed"] = [{"weight": module_state_dict["dist_bin_pairwise_embed.weight"]}]
    ...
    tbnm_state_dict["z_linear"] = [{"weight": module_state_dict["z_linear.weight"]}]
    ...
    for i in range(pairformer_num_blocks):
        ...
        for name in ["start", "end"]:
            tbnm_state_dict[f"pairformer_stack.layers.{i}.tri_attn_{name}.layer_norm"] = [
                {
            ...
            tbnm_state_dict[f"pairformer_stack.layers.{i}.tri_attn_{name}.linear"] = [
                {
                    "weight": module_state_dict[f"pairformer_stack.layers.{i}.tri_att_{name}.linear.weight"],
                }
            ]
            tbnm_state_dict[f"pairformer_stack.layers.{i}.tri_attn_{name}.mha.qkv_proj"] = [
                {
            ...
            tbnm_state_dict[f"pairformer_stack.layers.{i}.tri_attn_{name}.mha.o_proj"] = [
                {
                    "weight": module_state_dict[f"pairformer_stack.layers.{i}.tri_att_{name}.mha.linear_o.weight"],
                }
            ]
            tbnm_state_dict[f"pairformer_stack.layers.{i}.tri_attn_{name}.mha.g_proj"] = [
                {
        ...
        for name in ["z"]:
            tbnm_state_dict[f"pairformer_stack.layers.{i}.transition_{name}.norm"] = [
                {
            ...
            tbnm_state_dict[f"pairformer_stack.layers.{i}.transition_{name}.fused_fc2_fc1"] = [
                {
            ...
            tbnm_state_dict[f"pairformer_stack.layers.{i}.transition_{name}.fc3"] = [
                {
...
def _convert_pairformer_no_seq_block_torch(
    module_state_dict: dict, block_prefix: str, tbnm_prefix: str, out_dict: dict
) -> None:
...
def convert_hf_template_module_torch(
    ...
    tbnm_state_dict["z_proj"] = [{"weight": module_state_dict["z_proj.weight"]}]
    tbnm_state_dict["a_proj"] = [{"weight": module_state_dict["a_proj.weight"]}]
    tbnm_state_dict["u_proj"] = [{"weight": module_state_dict["u_proj.weight"]}]
