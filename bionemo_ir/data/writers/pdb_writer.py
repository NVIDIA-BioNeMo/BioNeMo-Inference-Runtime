...
``FoldingOutput`` schema as :class:`~bionemo_ir.data.writers.cif_writer.CIFWriter`,
...
from bionemo_ir.data.schemas.basic import AtomType, FoldingOutput, ResType
    ...
    * Lines are padded to 80 chars.
    ...
    def write(
        ...
        if residue_names is not None and len(residue_names) != n:
            raise ValueError(f"PDBWriter: residue_names length ({len(residue_names)}) must equal n_res ({n})")
        if mol_types is not None and len(mol_types) != n:
            raise ValueError(f"PDBWriter: mol_types length ({len(mol_types)}) must equal n_res ({n})")
        ...
        pdb_restypes: list[str] = [_IHM_REMAP.get(x.name, x.canonical_name) for x in self.res_types]
        ...
        for i in range(n):
            ...
            if residue_names is not None and i < len(residue_names):
                ...
                chain_to_pdb.setdefault(c, []).append(pdb_restypes[residue_types[i]])
        ...
        for c, seq in chain_to_seq.items():
            if mol_types is not None:
                ...
                chain_mol_types = {int(mol_types[i]) for i in chain_to_token_idx[c]}
                if len(chain_mol_types) != 1:
                    raise ValueError(
                        f"PDBWriter: mixed mol_types in chain {PDB_CHAIN_IDS[c]}: {sorted(chain_mol_types)}"
                    )
        ...
        chain_tag_of: dict[int, str] = {c: PDB_CHAIN_IDS[c] for c in unique_chains.tolist()}
        chain_is_het: dict[int, bool] = {c: chain_kind[c] == "nonpoly" for c in chain_kind}
        ...
        last_res_pdb_name: str | None = None
        last_res_index: int | None = None
        last_chain_tag: str | None = None
