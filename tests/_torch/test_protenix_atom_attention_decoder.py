...
def test_protenix_atom_attention_decoder(sc: Scenario):
    ...
    ref = RefProtenixAtomAttentionDecoderFromOSS.build(
    ...
    config = AtomAttentionDecoderConfig(c_token=sc.c_token,
                                        c_atom=sc.c_atom,
                                        c_atompair=sc.c_atompair,
                                        n_queries=sc.n_queries,
                                        n_keys=sc.n_keys,
    ...
    converted = convert_atom_attention_decoder_torch(config,
                                                     ref.state_dict(),
                                                     prefix="")
    ...
    atom_to_token_idx = (torch.arange(Na, device=device) //
                         (Na // Nt)).long().unsqueeze(0).expand(B, Na)
    ...
    with torch.inference_mode():
        ...
        act = model(atom_to_token_idx, a.to(torch_dtype),
                    q_skip.to(torch_dtype), c_skip.to(torch_dtype),
                    p_skip.to(torch_dtype))
