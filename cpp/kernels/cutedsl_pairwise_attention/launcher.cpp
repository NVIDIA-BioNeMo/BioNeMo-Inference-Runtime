...
CUtensorMapDataType tma_data_type(DType dtype)
{
  ...
  throw std::invalid_argument("unsupported pairwise-attention TMA dtype");
}

void encode_tma_descriptor(
  CUtensorMap& descriptor,
  embedded::TmaDescriptorInfo const& info,
  DType dtype,
  ...
  char const* name)
{
  ...
  if (info.rank != kSourceRank || info.rank > embedded::kTmaMaxRank)
    throw std::invalid_argument("pairwise-attention CUBIN has an unsupported TMA rank");
  if (info.data_type != tma_data_type(dtype))
    throw std::invalid_argument("pairwise-attention CUBIN has a mismatched TMA data type");
  validate_pointer(source.data, kTmaAddressAlignment, name);

  std::array<std::uint64_t, embedded::kTmaMaxRank> global_dimensions{};
  std::array<std::uint64_t, embedded::kTmaMaxRank - 1> global_strides{};
  ...
  for (std::uint32_t index = 0; index < info.rank; ++index)
  {
    ...
    if (source_index >= kSourceRank || seen_dimensions[source_index])
      throw std::invalid_argument("pairwise-attention CUBIN has an invalid TMA dimension order");
    ...
    if (global_dimensions[index] == 0 || info.box_dims[index] == 0 || info.element_strides[index] == 0)
      throw std::invalid_argument("pairwise-attention CUBIN has an invalid TMA extent");
    ...
    if (info.box_dims[index] > 256 || info.element_strides[index] > 8)
      throw std::invalid_argument("pairwise-attention CUBIN has out-of-range TMA traversal metadata");

    std::uint64_t const element_stride = source.strides[source_index];
    if (index == 0)
    {
      if (element_stride != 1)
        throw std::invalid_argument("pairwise-attention TMA innermost dimension must be contiguous");
      continue;
    }
    if (element_stride > std::numeric_limits<std::uint64_t>::max() / item_size)
      throw std::overflow_error("pairwise-attention TMA byte stride overflow");
  ...
  check_cuda_driver(
    cuTensorMapEncodeTiled(
      ...
      info.l2_promotion,
      info.oob_fill),
    "cuTensorMapEncodeTiled(pairwise_attention)");
}

std::uint64_t ceil_div(std::uint64_t value, std::uint64_t divisor)
{
  if (divisor == 0)
    throw std::invalid_argument("pairwise-attention launch divisor must be positive");
