...
def _broadcast_token_pair_to_blocks(
    z_token: torch.Tensor, atom_to_token_idx: torch.Tensor, K: int, W: int, attn_metadata: AttentionMetadata
) -> torch.Tensor:
    ...
    q_blocked = to_blocks(atom_to_token_idx.unsqueeze(-1).to(torch.float32), K, W)  # [B, K, W, 1]
    q_tok = q_blocked.squeeze(-1).long()  # [B, K, W]
    k_tok = attn_metadata.query_to_keys(q_blocked).squeeze(-1).long()  # [B,K,H]
...
def _expand_bs(x: torch.Tensor, B: int, S: int) -> torch.Tensor:
    ...
    return x.unsqueeze(1).expand(B, S, *x.shape[1:]).reshape(B * S, *x.shape[1:])
...
class ProtenixAtomAttentionEncoder(nn.Module):
    ...
    def __init__(self, config: BaseConfig) -> None:
        ...
        self.input_feature = {"ref_mask": 1, "ref_element": 128, "ref_atom_name_chars": 4 * 64}
        ...
        self.linear_no_bias_cl = Linear(c_atom, c_atompair, bias=False, dtype=dtype, skip_create_weights=skip)
        self.linear_no_bias_cm = Linear(c_atom, c_atompair, bias=False, dtype=dtype, skip_create_weights=skip)
        if self.has_coords:
            ...
            self.layernorm_s = nn.LayerNorm(config.c_s, bias=False, eps=eps, dtype=dtype)
            self.linear_no_bias_s = Linear(config.c_s, c_atom, bias=False, dtype=dtype, skip_create_weights=skip)
            self.layernorm_z = nn.LayerNorm(config.c_z, bias=False, eps=eps, dtype=dtype)
            self.linear_no_bias_z = Linear(config.c_z, c_atompair, bias=False, dtype=dtype, skip_create_weights=skip)
            self.linear_no_bias_r = Linear(3, c_atom, bias=False, dtype=dtype, skip_create_weights=skip)
        self.small_mlp = nn.Sequential(
            nn.ReLU(),
            Linear(c_atompair, c_atompair, bias=False, dtype=dtype, skip_create_weights=skip),
            nn.ReLU(),
            Linear(c_atompair, c_atompair, bias=False, dtype=dtype, skip_create_weights=skip),
            nn.ReLU(),
            Linear(c_atompair, c_atompair, bias=False, dtype=dtype, skip_create_weights=skip),
    ...
    def forward(
        ...
        d = self.dtype
        ref_pos, ref_charge, ref_mask = ref_pos.to(d), ref_charge.to(d), ref_mask.to(d)
        ...
        p_lm, c_l = self.prepare_cache(
            ref_pos, ref_charge, ref_mask, ref_element, ref_atom_name_chars, atom_to_token_idx, d_lm, v_lm, pad_info
        )
    ...
    def _prepare_coords(
        ...
        z_block = _broadcast_token_pair_to_blocks(z_proj, a2t, K, W, attn_metadata)
