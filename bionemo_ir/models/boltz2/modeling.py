...
from bionemo_ir._torch.modules.boltz.confidence import Boltz2ConfidenceModule
from bionemo_ir._torch.modules.boltz.confidence_utils import compute_contact_prob
from bionemo_ir._torch.modules.boltz.embedders import Boltz2InputEmbedder
...
from bionemo_ir.pipeline.models.boltz2.const import contact_conditioning_info, num_bond_types
...
class Boltz2(nn.Module, OptimizedModuleSetterMixin):
    ...
    def forward(
        ...
        del network_condition_kwargs, diffusion_output
...
class Boltz2Affinity(Boltz2, OptimizedModuleSetterMixin):
    def __init__(
        ...
        if include_load_weights:
            self.load_weights()
        # ``Boltz2.__init__`` rewrote the base model before these affinity
        # modules existed, so apply the idempotent rewrite once more.
        replace_with_fused_layernorm(self)
