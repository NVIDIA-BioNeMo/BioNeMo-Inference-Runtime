...
from test_utils.openfold.ref_layers import (
    ...
    RefTemplatePairStackBlock,
    RefTemplatePointwiseAttention,
)
# isort: on


def create_msa_attention_weights(
    c_in=None,
    c_hidden=None,
    no_heads=None,
    pair_bias=False,
    ...
    if not from_ref:
        ...
        if not using_tri_attn:
            ...
            mha_weights = create_triangle_attention_weights(
                c_q=c_in, c_k=c_in, c_v=c_in, bias_flags={"q": False, "k": False, "v": False, "g": True, "o": True}
            )
    else:
        ...
        if not from_ref.using_tri_attn:
            mha_weights = create_self_pairwise_attention_weights(from_ref=from_ref.mha)
        else:
...
def load_msa_attention_weights_ref_torch(module, weights_and_biases):
    layer_norm_m_weight, layer_norm_m_bias, layer_norm_z_weight, layer_norm_z_bias, linear_z_weight, mha_weights = (
        weights_and_biases
    )
...
def load_msa_attention_weights_torch(module, weights_and_biases):
    ...
    if linear_z_weight is not None:
        module.proj_z.load_weights([{"weight": linear_z_weight.to("cuda"), "bias": None}])


def load_msa_attention_weights_trt(module, weights_and_biases, using_tri_attn: bool = True):
    layer_norm_m_weight, layer_norm_m_bias, layer_norm_z_weight, layer_norm_z_bias, linear_z_weight, mha_weights = (
        weights_and_biases
    )
    ...
    module.layer_norm_m.weight.value = np.ascontiguousarray(layer_norm_m_weight.cpu().numpy())
    module.layer_norm_m.bias.value = np.ascontiguousarray(layer_norm_m_bias.cpu().numpy())
    if layer_norm_z_weight is not None:
        module.proj_z_norm.weight.value = np.ascontiguousarray(layer_norm_z_weight.cpu().numpy())
        module.proj_z_norm.bias.value = np.ascontiguousarray(layer_norm_z_bias.cpu().numpy())
    if linear_z_weight is not None:
        module.proj_z.weight.value = np.ascontiguousarray(linear_z_weight.cpu().numpy())
...
def load_msa_transition_weights_ref_torch(module, weights_and_biases):
    layer_norm_weight, layer_norm_bias, linear_1_weight, linear_1_bias, linear_2_weight, linear_2_bias = (
        weights_and_biases
    )
...
def load_msa_transition_weights_torch(module, weights_and_biases):
    ...
    module.linear_1.load_weights(
        [
            {
                "weight": linear_1_weight.to("cuda"),
                "bias": linear_1_bias.to("cuda") if linear_1_bias is not None else None,
            }
        ]
    )
    module.linear_2.load_weights(
        [
            {
                "weight": linear_2_weight.to("cuda"),
                "bias": linear_2_bias.to("cuda") if linear_2_bias is not None else None,
            }
        ]
    )
...
def load_msa_transition_weights_trt(module, weights_and_biases):
    ...
    module.linear_1.weight.value = np.ascontiguousarray(linear_1_weight.cpu().numpy())
    module.linear_1.bias.value = np.ascontiguousarray(linear_1_bias.cpu().numpy())
    module.linear_2.weight.value = np.ascontiguousarray(linear_2_weight.cpu().numpy())
    module.linear_2.bias.value = np.ascontiguousarray(linear_2_bias.cpu().numpy())
...
def create_evoformer_block_weights(from_ref: RefEvoformerBlock = None):
    msa_att_row_weights = create_msa_attention_weights(from_ref=from_ref.msa_att_row)
    msa_att_col_weights = create_msa_attention_weights(from_ref=from_ref.msa_att_col)
    msa_transition_weights = create_msa_transition_weights(from_ref=from_ref.msa_transition)
    outer_product_mean_weights = create_outer_product_mean_weights(from_ref=from_ref.outer_product_mean)
    tri_mul_out_weights = create_triangle_multiplication_node_weights(from_ref=from_ref.tri_mul_out)
    tri_mul_in_weights = create_triangle_multiplication_node_weights(from_ref=from_ref.tri_mul_in)
    tri_attn_start_weights = create_triangle_attention_node_weights(from_ref=from_ref.tri_attn_start)
    tri_attn_end_weights = create_triangle_attention_node_weights(from_ref=from_ref.tri_attn_end)
...
def load_evoformer_block_weights_ref_torch(module, weights_and_biases):
    ...
    load_msa_attention_weights_ref_torch(module.msa_att_row, msa_att_row_weights)
    load_msa_attention_weights_ref_torch(module.msa_att_col, msa_att_col_weights)
    load_msa_transition_weights_ref_torch(module.msa_transition, msa_transition_weights)
    load_outer_product_mean_weights_ref_torch(module.outer_product_mean, outer_product_mean_weights)
    load_triangle_multiplication_node_weights_ref_torch(module.tri_mul_out, tri_mul_out_weights)
    load_triangle_multiplication_node_weights_ref_torch(module.tri_mul_in, tri_mul_in_weights)
    load_triangle_attention_node_weights_ref_torch(module.tri_attn_start, tri_attn_start_weights)
    load_triangle_attention_node_weights_ref_torch(module.tri_attn_end, tri_attn_end_weights)
    load_pair_transition_weights_ref_torch(module.pair_transition, pair_transition_weights)
...
def load_evoformer_block_weights_torch(module, weights_and_biases):
    ...
    load_msa_transition_weights_torch(module.msa_transition, msa_transition_weights)
    load_outer_product_mean_weights_torch(module.outer_product_mean, outer_product_mean_weights)
    load_triangle_multiplication_node_weights_torch(module.tri_mul_out, tri_mul_out_weights)
    load_triangle_multiplication_node_weights_torch(module.tri_mul_in, tri_mul_in_weights)
    load_triangle_attention_node_weights_torch(module.tri_attn_start, tri_attn_start_weights)
    load_triangle_attention_node_weights_torch(module.tri_attn_end, tri_attn_end_weights)
    load_pair_transition_weights_torch(module.pair_transition, pair_transition_weights)
...
def load_evoformer_block_weights_trt(module, weights_and_biases):
    ...
    load_msa_transition_weights_trt(module.msa_transition, msa_transition_weights)
    load_outer_product_mean_weights_trt(module.outer_product_mean, outer_product_mean_weights)
    load_triangle_multiplication_node_weights_trt(module.tri_mul_out, tri_mul_out_weights)
    load_triangle_multiplication_node_weights_trt(module.tri_mul_in, tri_mul_in_weights)
    load_triangle_attention_node_weights_trt(module.tri_attn_start, tri_attn_start_weights)
    load_triangle_attention_node_weights_trt(module.tri_attn_end, tri_attn_end_weights)
    load_pair_transition_weights_trt(module.pair_transition, pair_transition_weights)


def create_msa_global_attention_weights(from_ref: RefMSAColumnGlobalAttention = None):
    layer_norm_m_weight = from_ref.layer_norm_m.weight.data
    layer_norm_m_bias = from_ref.layer_norm_m.bias.data
...
def load_msa_global_attention_weights_ref_torch(module, weights_and_biases):
    ...
    module.layer_norm_m.weight.data.copy_(layer_norm_m_weight.to("cuda"))
    module.layer_norm_m.bias.data.copy_(layer_norm_m_bias.to("cuda"))
    module.global_attention.linear_q.weight.data.copy_(linear_q_weight.to("cuda"))
    module.global_attention.linear_k.weight.data.copy_(linear_k_weight.to("cuda"))
    module.global_attention.linear_v.weight.data.copy_(linear_v_weight.to("cuda"))
    module.global_attention.linear_g.weight.data.copy_(linear_g_weight.to("cuda"))
    module.global_attention.linear_g.bias.data.copy_(linear_g_bias.to("cuda"))
    module.global_attention.linear_o.weight.data.copy_(linear_o_weight.to("cuda"))
...
def load_msa_global_attention_weights_torch(module, weights_and_biases):
    ...
    module.layer_norm_m.weight.data.copy_(layer_norm_m_weight.to("cuda"))
    module.layer_norm_m.bias.data.copy_(layer_norm_m_bias.to("cuda"))
    module.global_attention.proj_q.load_weights([{"weight": linear_q_weight.to("cuda"), "bias": None}])
    module.global_attention.fused_proj_kv.load_weights(
        [{"weight": linear_k_weight.to("cuda"), "bias": None}, {"weight": linear_v_weight.to("cuda"), "bias": None}]
    )
    module.global_attention.proj_g.load_weights(
        [{"weight": linear_g_weight.to("cuda"), "bias": linear_g_bias.to("cuda")}]
    )
    module.global_attention.proj_o.load_weights(
        [{"weight": linear_o_weight.to("cuda"), "bias": linear_o_bias.to("cuda")}]
    )


def create_extra_msa_block_weights(from_ref: RefExtraMSABlock = None):
    msa_att_row_weights = create_msa_attention_weights(from_ref=from_ref.msa_att_row)
    msa_att_col_weights = create_msa_global_attention_weights(from_ref=from_ref.msa_att_col)
    msa_transition_weights = create_msa_transition_weights(from_ref=from_ref.msa_transition)
    outer_product_mean_weights = create_outer_product_mean_weights(from_ref=from_ref.outer_product_mean)
    tri_mul_out_weights = create_triangle_multiplication_node_weights(from_ref=from_ref.tri_mul_out)
    tri_mul_in_weights = create_triangle_multiplication_node_weights(from_ref=from_ref.tri_mul_in)
...
def load_extra_msa_block_weights_ref_torch(module, weights_and_biases):
    ...
    load_msa_attention_weights_ref_torch(module.msa_att_row, msa_att_row_weights)
    load_msa_global_attention_weights_ref_torch(module.msa_att_col, msa_att_col_weights)
    load_msa_transition_weights_ref_torch(module.msa_transition, msa_transition_weights)
    load_outer_product_mean_weights_ref_torch(module.outer_product_mean, outer_product_mean_weights)
    load_triangle_multiplication_node_weights_ref_torch(module.tri_mul_out, tri_mul_out_weights)
    load_triangle_multiplication_node_weights_ref_torch(module.tri_mul_in, tri_mul_in_weights)
...
def load_extra_msa_block_weights_torch(module, weights_and_biases):
    ...
    load_msa_attention_weights_torch(module.msa_att_row, msa_att_row_weights)
    load_msa_global_attention_weights_torch(module.msa_att_col, msa_att_col_weights)
    load_msa_transition_weights_torch(module.msa_transition, msa_transition_weights)
    load_outer_product_mean_weights_torch(module.outer_product_mean, outer_product_mean_weights)
    load_triangle_multiplication_node_weights_torch(module.tri_mul_out, tri_mul_out_weights)
    load_triangle_multiplication_node_weights_torch(module.tri_mul_in, tri_mul_in_weights)
...
def create_input_embedder_weights(from_ref: RefInputEmbedder = None):
    ...
    return (
        linear_tf_z_i_weight,
        linear_tf_z_i_bias,
        linear_tf_z_j_weight,
        linear_tf_z_j_bias,
        linear_tf_m_weight,
        linear_tf_m_bias,
        linear_msa_m_weight,
        linear_msa_m_bias,
        linear_relpos_weight,
        linear_relpos_bias,
    )
...
def load_input_embedder_weights_torch(module, weights_and_biases):
    ...
    module.fused_linear_tf_z.load_weights(
        [
            {"weight": linear_tf_z_i_weight.to("cuda"), "bias": linear_tf_z_i_bias.to("cuda")},
            {"weight": linear_tf_z_j_weight.to("cuda"), "bias": linear_tf_z_j_bias.to("cuda")},
        ]
    )
    module.linear_tf_m.load_weights([{"weight": linear_tf_m_weight.to("cuda"), "bias": linear_tf_m_bias.to("cuda")}])
    module.linear_msa_m.load_weights([{"weight": linear_msa_m_weight.to("cuda"), "bias": linear_msa_m_bias.to("cuda")}])
    module.linear_relpos.load_weights(
        [{"weight": linear_relpos_weight.to("cuda"), "bias": linear_relpos_bias.to("cuda")}]
    )
...
def load_recycling_embedder_weights_torch(module, weights_and_biases):
    linear_weight, linear_bias, layer_norm_m_weight, layer_norm_m_bias, layer_norm_z_weight, layer_norm_z_bias = (
        weights_and_biases
    )

    module.linear.load_weights([{"weight": linear_weight.to("cuda"), "bias": linear_bias.to("cuda")}])
    module.layer_norm_m.weight.data.copy_(layer_norm_m_weight.to("cuda"))
    module.layer_norm_m.bias.data.copy_(layer_norm_m_bias.to("cuda"))
    module.layer_norm_z.weight.data.copy_(layer_norm_z_weight.to("cuda"))
    module.layer_norm_z.bias.data.copy_(layer_norm_z_bias.to("cuda"))
...
def create_template_pair_stack_block_weights(from_ref: RefTemplatePairStackBlock = None):
    tri_mul_out_weights = create_triangle_multiplication_node_weights(from_ref=from_ref.tri_mul_out)
    tri_mul_in_weights = create_triangle_multiplication_node_weights(from_ref=from_ref.tri_mul_in)
    tri_attn_start_weights = create_triangle_attention_node_weights(from_ref=from_ref.tri_attn_start)
    tri_attn_end_weights = create_triangle_attention_node_weights(from_ref=from_ref.tri_attn_end)
...
def create_template_pointwise_attention_weights(from_ref: RefTemplatePointwiseAttention = None):
    mha_weights = create_triangle_attention_weights(from_ref=from_ref.mha)
    return mha_weights
