...
class Boltz2FinalFeatureCollator(FeatureCollatorBase):
    ...
    def __call__(
        ...
        # Ligand tokens use the model's UNK restype by design. Preserve their
        # original CCD names as writer-only metadata before the feature stage
        # replaces the tokenizer row. Boltz1 shares this collator.
        row = context.get("_row") or context
        tokens = row.get("tokens") if isinstance(row, dict) else None
        if tokens is not None:
            features["token_resnames"] = [
                token["res_name"] if isinstance(token, dict) else token.res_name for token in tokens
            ]
