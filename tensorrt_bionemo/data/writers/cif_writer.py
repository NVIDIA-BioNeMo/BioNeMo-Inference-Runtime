...
import numpy as np
import modelcif
from modelcif import qa_metric, model, dumper
from modelcif.qa_metric import Local, Global, PLDDT
from modelcif.model import AbInitioModel
...
class CIFWriter(BaseWriter):
    ...
    def write(self, folding_output: FoldingOutput) -> str:
        ...
        last_chain_idx = None
        for i in range(n):
            if last_chain_idx is not None and last_chain_idx != chain_indices[i]:
        ...
        class _MyModel(modelcif.model.AbInitioModel):
                ...
                for i in range(n):
                    for atom_name, pos, mask, b_factor in zip(
                        ...
                        yield modelcif.model.Atom(
                            asym_unit=asym_unit_map[chain_indices[i]], type_symbol=element,
                            seq_id=residue_indices[i], atom_id=atom_name,
            ...
            def add_scores(self):
                ...
                for i in range(n):
                    for mask, b_factor in zip(atom_mask[i], b_factors[i]):
                        ...
                        if chain_indices[i] not in plddt_per_residue:
                            ...
                            plddt_per_residue[chain_indices[i]] = {residue_indices[i]: b_factor}
                        if residue_indices[i] not in plddt_per_residue[chain_indices[i]]:
                            plddt_per_residue[chain_indices[i]][residue_indices[i]] = b_factor
                plddts = []
                ...
                # global score
                self.qa_metrics.append((_GlobalPLDDT(np.mean(np.array(plddts)))))
