...
def convert_hf_pairformer(config: PairformerConfig, local_checkpoint: str = None, model_name: str = "openfold3"):
                ...
                if "tri_mul_out" in name or "tri_mul_in" in name:
                    ...
                    if "linear_a_p" in name or "linear_b_p" in name:
                        ...
                        if p_in_name not in pairformer_state_dict:
                            ...
                            pairformer_state_dict[p_in_name] = torch.cat([p0, p1], dim=0)
                        continue
                    if "linear_a_g" in name or "linear_b_g" in name:
                        ...
                        if g_in_name not in pairformer_state_dict:
                            ...
                            pairformer_state_dict[g_in_name] = torch.cat([g0, g1], dim=0)
                        continue
    ...
    for i in range(config.num_blocks):
        ...
        weights.update(
            get_tri_attn_node_weights(
                ...
                f"{layer_bioir_prefix}.tri_attn_start",
                dtype=config.dtype,
            )
        )
        weights.update(
            get_tri_attn_node_weights(
                ...
                f"{layer_bioir_prefix}.tri_attn_end",
                dtype=config.dtype,
            )
        )
        weights.update(
            get_tri_mul_node_weights(
                ...
                f"{layer_bioir_prefix}.tri_mul_out",
                dtype=config.dtype,
            )
        )
        weights.update(
            get_tri_mul_node_weights(
                ...
                f"{layer_bioir_prefix}.tri_mul_in",
                dtype=config.dtype,
            )
        )
        if not config.no_update_s:
...
def convert_hf_pairformer_torch(
    config: PairformerConfig, local_checkpoint: str = None, model_name: str = "openfold3", weights: dict = None
):
    if weights is None:
        state_dict = load_weights(name=model_name, cache_path=local_checkpoint)
    else:
...
def get_conditioned_transition_block_weights(
    state_dict: dict,
    prefix: str,
    bioir_prefix: str,
    dim: int,
    ...
    ret = {}
    ret.update(
        get_adaln_weights(state_dict, f"{prefix}.adaln", f"{bioir_prefix}.adaln", dim, dim_single_cond, dtype=dtype)
    )
    ...
    output_projection_weight = state_dict[f"{prefix}.output_projection.0.weight"]
    output_projection_bias = state_dict[f"{prefix}.output_projection.0.bias"]

    fused_swl_a_to_b_weight = torch.cat([swish_gate_weight, a_to_b_weight], dim=0)
    ret.update(
        {
            f"{bioir_prefix}.fused_swl_a_to_b.weight": fused_swl_a_to_b_weight.to(torch_dtype),
            f"{bioir_prefix}.b_to_a.weight": b_to_a_weight.to(torch_dtype),
            f"{bioir_prefix}.output_projection.weight": output_projection_weight.to(torch_dtype),
            f"{bioir_prefix}.output_projection.bias": output_projection_bias.to(torch_dtype),
        }
    )
    return ret


def convert_hf_diffusion_transformer(
    config: DiffusionTransformerConfig, local_checkpoint: str = None, model_name: str = "openfold3"
):
    state_dict = load_weights(name=model_name, cache_path=local_checkpoint)
    assert state_dict is not None
        ...
        if name.startswith(prefix):
            ...
            if "attention_pair_bias" in name:
                ...
                if "layer_norm_a" in name:
                    name = name.replace("attention_pair_bias.layer_norm_a", "adaln")
                ...
                if "linear_ada_out" in name:
                    name = name.replace("attention_pair_bias.linear_ada_out", "output_projection.0")
                    tktn_state_dict[name] = param
                    continue
                if "mha" in name:
                    name = name.replace("attention_pair_bias.mha", "pair_bias_attn")
                ...
                if "layer_norm_z" in name or "linear_z" in name:
                    name = name.replace("attention_pair_bias.layer_norm_z", "pair_bias_attn.proj_z.0")
                    name = name.replace("attention_pair_bias.linear_z", "pair_bias_attn.proj_z.1")
                    tktn_state_dict[name] = param
                    continue
                logger.warning(f"Miss converting the weight: {name}")
            if "conditioned_transition" in name:
                if "layer_norm." in name:
                    name = name.replace("conditioned_transition.layer_norm", "transition.adaln")
                    name = name.replace("layer_norm_s", "s_norm")
                    name = name.replace("linear_g", "s_scale")
                    name = name.replace("linear_s", "s_bias")
                    tktn_state_dict[name] = param
                ...
                if "swiglu" in name:
                    name = name.replace(
                        "conditioned_transition.swiglu.linear_a.weight", "transition.swish_gate.0.weight"
                    )
                    name = name.replace("conditioned_transition.swiglu.linear_b.weight", "transition.a_to_b.weight")
                    tktn_state_dict[name] = param
                    continue
                if "linear_out" in name or "linear_g" in name:
                    name = name.replace("conditioned_transition.linear_out", "transition.b_to_a")
                    name = name.replace("conditioned_transition.linear_g", "transition.output_projection.0")
                    tktn_state_dict[name] = param
                    continue
                logger.warning(f"Miss converting the weight: {name}")
    ...
    for i in range(config.num_blocks):
        ...
        weights.update(
            get_pairwise_attn_weights(
                ...
                f"{layer_bioir_prefix}.pair_bias_attn",
        ...
        weights.update(
            get_conditioned_transition_block_weights(
                ...
                f"{layer_bioir_prefix}.transition",
                config.dim,
                config.dim_single_cond,
        ...
        weights.update(
            get_output_projection_weights(
                ...
                f"{layer_bioir_prefix}.output_projection",
                dtype=config.dtype,
            )
        )
        weights.update(
            get_post_norm_weights(
                tktn_state_dict, f"{layer_prefix}.post_lnorm", f"{layer_bioir_prefix}.post_lnorm", dtype=config.dtype
            )
        )
...
def _template_module_weight(bioir_state_dict, module_state_dict, name):
    bioir_state_dict[name] = [
        {"weight": module_state_dict[f"{name}.weight"], "bias": module_state_dict.get(f"{name}.bias", None)}
    ]
...
def convert_hf_diffusion_transformer_torch(
    ...
    **kwargs,
            ...
            if "attention_pair_bias" in name:
                ...
                if config.use_separate_layer_norm:
                    if "layer_norm_a_q" in name:
                        name = name.replace("attention_pair_bias.layer_norm_a_q", "pair_bias_attn.layer_norm_a_q")
                        name = name.replace("layer_norm_s.", "s_norm.")
                        name = name.replace("linear_g.", "s_scale.")
                        name = name.replace("linear_s.", "s_bias.")
                    ...
                    if "layer_norm_a_k" in name:
                        name = name.replace("attention_pair_bias.layer_norm_a_k", "pair_bias_attn.layer_norm_a_k")
                ...
                if "layer_norm_z" in name or "linear_z" in name:
                    if getattr(config, "shared_pair_norm", False):
                        ...
                        if "linear_z" in name:
                            name = name.replace("attention_pair_bias.linear_z", "pair_bias_attn.proj_z.0")
                            module_state_dict[name] = param
    ...
    bioir_state_dict = {}
    dim = module_state_dict["layers.0.pair_bias_attn.proj_q.weight"].shape[0]
    dtype = module_state_dict["layers.0.pair_bias_attn.proj_q.weight"].dtype
    ...
    for i in range(config.num_blocks):
        if not config.use_separate_layer_norm:
            ...
            bioir_state_dict[f"layers.{i}.adaln.fused_s_scale_s_bias"] = [
                ...
                {
                    "weight": module_state_dict[f"layers.{i}.adaln.s_bias.weight"],
                    "bias": torch.zeros([dim], dtype=dtype),
                },
            ]
        else:
            bioir_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_q.a_norm"] = [
                {"weight": torch.ones([dim], dtype=dtype)}
            ]
            _template_module_weight(
                bioir_state_dict, module_state_dict, f"layers.{i}.pair_bias_attn.layer_norm_a_q.s_norm"
            )

            bioir_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_q.fused_s_scale_s_bias"] = [
                {
                    "weight": module_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_q.s_scale.weight"],
                    "bias": module_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_q.s_scale.bias"],
                },
                {
                    "weight": module_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_q.s_bias.weight"],
                    "bias": torch.zeros([dim], dtype=dtype),
                },
            ]

            bioir_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_k.a_norm"] = [
                {"weight": torch.ones([dim], dtype=dtype)}
            ]
            _template_module_weight(
                bioir_state_dict, module_state_dict, f"layers.{i}.pair_bias_attn.layer_norm_a_k.s_norm"
            )

            bioir_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_k.fused_s_scale_s_bias"] = [
                {
                    "weight": module_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_k.s_scale.weight"],
                    "bias": module_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_k.s_scale.bias"],
                },
                {
                    "weight": module_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_k.s_bias.weight"],
                    "bias": torch.zeros([dim], dtype=dtype),
                },
            ]
...
def convert_hf_input_embedder_torch(
    config: BaseConfig, local_checkpoint: str = None, model_name: str = "openfold3", weights: dict = None, **kwargs
):
    if weights is None:
        state_dict = load_weights(name=model_name, cache_path=local_checkpoint)
    else:
    ...
    module_state_dict = {}
    ref_atom_attn_enc_weight_list = ["linear_l", "linear_m", "pair_mlp.1", "pair_mlp.3", "pair_mlp.5", "linear_q.0"]

    for layer_name in ref_atom_attn_enc_weight_list:
        module_state_dict[f"atom_attn_enc.{layer_name}"] = [
            {
                "weight": state_dict[f"input_embedder.atom_attn_enc.{layer_name}.weight"],
                "bias": state_dict.get(f"input_embedder.atom_attn_enc.{layer_name}.bias", None),
            }
        ]

    module_state_dict["atom_attn_enc.ref_atom_feature_embedder.linear_ref_pair_features"] = [
        {
            "weight": state_dict["input_embedder.atom_attn_enc.ref_atom_feature_embedder.linear_ref_offset.weight"],
            "bias": None,
        },
        {
            "weight": state_dict["input_embedder.atom_attn_enc.ref_atom_feature_embedder.linear_inv_sq_dists.weight"],
            "bias": None,
        },
        {
            "weight": state_dict["input_embedder.atom_attn_enc.ref_atom_feature_embedder.linear_valid_mask.weight"],
            "bias": None,
        },
    ]
    ...
    module_state_dict["atom_attn_enc.ref_atom_feature_embedder.linear_merge_ref_features"] = merge_weight
    ...
    if getattr(config.atom_transformer_config, "shared_pair_norm", False):
        ...
        layer_norm_z_w = state_dict["input_embedder.atom_attn_enc.atom_transformer.layer_norm_z.weight"]
        module_state_dict["atom_attn_enc.atom_transformer.layer_norm_z"] = [{"weight": layer_norm_z_w}]

    input_embedder_weight_list = ["linear_s", "linear_relpos", "linear_token_bonds"]
    for layer_name in input_embedder_weight_list:
        module_state_dict[f"{layer_name}"] = [
            {
                "weight": state_dict[f"input_embedder.{layer_name}.weight"],
                "bias": state_dict.get(f"input_embedder.{layer_name}.bias", None),
            }
        ]
...
def convert_hf_template_embedder_torch(
        ...
        if name.startswith(prefix) and ".weight" in name:
            ...
            for replace_name, replace_name_ in replace_name_dict.items():
                if replace_name in layer_name:
                    layer_name = layer_name.replace(replace_name, replace_name_)

            module_state_dict[layer_name.replace(".weight", "")] = [
                {
                    "weight": state_dict[name],
                    "bias": state_dict.get(name.replace(".weight", ".bias"), None),
                }
            ]

    merge_weights_list = {
        "tri_mul_out.p_in": ["tri_mul_out.linear_a_p", "tri_mul_out.linear_b_p"],
        ...
        for name_ in module_state_dict.keys():
            ...
            if start_name in name_:
                ...
                for same_group_name in value[1:]:
                    merge_layer_name = name_.replace(start_name, same_group_name)
    ...
    template_pair_embedder_merge_feats_weight_list = [
        ...
        "template_pair_embedder.x_linear",
        "template_pair_embedder.y_linear",
    ...
    for layer_name in template_pair_embedder_merge_feats_weight_list:
        merge_weight.append(
            {
                "weight": state_dict[f"template_embedder.{layer_name}.weight"],
                "bias": state_dict.get(f"template_embedder.{layer_name}.bias", None),
            }
        )
...
def convert_hf_msa_stack_torch(
    ...
    assert state_dict is not None
    prefix = "msa_module."
    # This part is to replace the names of the weights to the names of the BioIR model
    module_state_dict = {}
    replace_name_dict = {
    ...
    merge_weights_list = {
        "msa_att_row.fused_proj_m_g": ["msa_att_row.linear_v", "msa_att_row.linear_g"],
        "msa_transition.fused_fc2_fc1": ["msa_transition.swiglu.linear_b", "msa_transition.swiglu.linear_a"],
        "outer_product_mean.fused_proj_a_b": ["outer_product_mean.linear_1", "outer_product_mean.linear_2"],
        "tri_mul_out.p_in": ["pair_stack.tri_mul_out.linear_a_p", "pair_stack.tri_mul_out.linear_b_p"],
...
def convert_hf_auxiliary_heads_torch(
    config: BaseConfig, local_checkpoint: str = None, model_name: str = "openfold3", weights: dict = None, **kwargs
):
    if weights is None:
        state_dict = load_weights(name=model_name, cache_path=local_checkpoint)
...
def convert_hf_diffusion_module_torch(
    config: BaseConfig, local_checkpoint: str = None, model_name: str = "openfold3", weights: dict = None, **kwargs
):
    if weights is None:
        state_dict = load_weights(name=model_name, cache_path=local_checkpoint)
    ...
    replace_name_dict = {
        "transition_z.0.layer_norm": "transition_z.0.norm",
        "transition_z.0.linear_out": "transition_z.0.fc3",
        "transition_z.1.layer_norm": "transition_z.1.norm",
        "transition_z.1.linear_out": "transition_z.1.fc3",
        "transition_s.0.layer_norm": "transition_s.0.norm",
        "transition_s.0.linear_out": "transition_s.0.fc3",
        "transition_s.1.layer_norm": "transition_s.1.norm",
        "transition_s.1.linear_out": "transition_s.1.fc3",
        "diffusion_conditioning.fourier_emb": "diffusion_conditioning.fourier_emb.proj",
    }
    ignore_layer_list = ["sample_diffusion", "diffusion_transformer", "attention_pair_bias", "atom_transformer"]
        ...
        if name.startswith(prefix) and (".weight" in name or ".w" in name):
            ...
            if "fourier" not in layer_name:
                ...
                module_state_dict[layer_name.replace(".w", "")] = [
                    {
                        "weight": fourier_emb_weight,
                        "bias": state_dict.get(name.replace(".w", ".b"), None),
                    }
                ]

    merge_weights_list = {
        "transition_z.0.fused_fc2_fc1": ["transition_z.0.swiglu.linear_b", "transition_z.0.swiglu.linear_a"],
        "transition_z.1.fused_fc2_fc1": ["transition_z.1.swiglu.linear_b", "transition_z.1.swiglu.linear_a"],
        "transition_s.0.fused_fc2_fc1": ["transition_s.0.swiglu.linear_b", "transition_s.0.swiglu.linear_a"],
        "transition_s.1.fused_fc2_fc1": ["transition_s.1.swiglu.linear_b", "transition_s.1.swiglu.linear_a"],
    }
    clean_layer_list = []
    merge_layer_dict = {}
    ...
    for key, value in atom_attn_enc_coder.items():
        atom_attn_enc_coder_weights[f"atom_attn_enc.atom_transformer.{key}"] = value
    ...
    for key, value in diffusion_transformer_coder.items():
        diffusion_transformer_coder_weights[f"diffusion_transformer.{key}"] = value
    ...
    for key, value in atom_attn_dec_coder.items():
        atom_attn_dec_coder_weights[f"atom_attn_dec.atom_transformer.{key}"] = value
...
def convert_hf_openfold3_torch(
    config: BaseConfig, local_checkpoint: str = None, model_name: str = "openfold3", weights: dict = None, **kwargs
):
    if weights is None:
        state_dict = load_weights(name=model_name, cache_path=local_checkpoint)
    ...
    input_embedder_weights = convert_hf_input_embedder_torch(
        config=config.input_embedder_config, model_name=model_name, weights=state_dict
    )
    msa_stack_weights = convert_hf_msa_stack_torch(
        config=config.msa_stack_module_config, model_name=model_name, weights=state_dict
    )
    msa_module_embedder_weights = convert_hf_msa_module_embedder_torch(
        config=config.msa_module_embedder_config, model_name=model_name, weights=state_dict
    )
    diffusion_module_weights = convert_hf_diffusion_module_torch(
        config=config.diffusion_module_config, model_name=model_name, weights=state_dict
    )
    template_embedder_weights = convert_hf_template_embedder_torch(
        config=config.template_embedder_config, model_name=model_name, weights=state_dict
    )
    pairformer_stack_weights = convert_hf_pairformer_torch(
        config=config.trunk.pairformer, model_name=model_name, weights=state_dict
    )
    auxiliary_heads_weights = convert_hf_auxiliary_heads_torch(
        config=config.auxiliary_heads_config, model_name=model_name, weights=state_dict
    )
    layer_norm_z_weights = {"weight": state_dict["layer_norm_z.weight"], "bias": state_dict["layer_norm_z.bias"]}
    layer_norm_s_weights = {"weight": state_dict["layer_norm_s.weight"], "bias": state_dict["layer_norm_s.bias"]}
    linear_z_weights = [{"weight": state_dict["linear_z.weight"], "bias": None}]
    linear_s_weights = [{"weight": state_dict["linear_s.weight"], "bias": None}]
