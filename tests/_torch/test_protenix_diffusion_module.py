...
from tensorrt_bionemo._torch.modules.protenix import (ProtenixDiffusionModule,
...
from tensorrt_bionemo.models.protenix.config import (
    AtomAttentionDecoderConfig, DiffusionAtomAttentionEncoderConfig,
    DiffusionConditioningConfig, DiffusionModuleConfig,
...
def _atom_tc(sc: Scenario, dtype: str) -> DiffusionTransformerConfig:
    return DiffusionTransformerConfig(num_blocks=sc.atom_n_blocks,
...
def _token_tc(sc: Scenario,
              dtype: str,
              backend: str = "SDPA",
              precompute_bias: bool = True) -> DiffusionTransformerConfig:
    return DiffusionTransformerConfig(num_blocks=sc.token_n_blocks,
...
@pytest.mark.parametrize("precompute_bias", [False, True],
                         ids=["per_layer_bias", "mega_bias"])
def test_token_transformer_broadcasts_sample_independent_pair(
        backend: str, precompute_bias: bool):
    """Match sample-independent pair broadcast to explicit ``B*S`` expansion."""
    skip_if_cutedsl(backend)
    torch.manual_seed(123)
    device = torch.device("cuda")
    dtype = torch.bfloat16
    sc = Scenario(c_token=128,
                  c_s=128,
                  c_z=16,
                  token_n_blocks=2,
                  token_n_heads=4,
                  dtype="bfloat16")
    model = ProtenixDiffusionTransformer(
        _token_tc(sc,
                  "bfloat16",
                  backend=backend,
                  precompute_bias=precompute_bias)).to(device).eval()
    ...
    with torch.inference_mode():
        broadcast = model(a.reshape(B * S, N, sc.c_token),
                          s.reshape(B * S, N, sc.c_s), z,
                          mask).reshape(B, S, N, sc.c_token)

        # Explicit baseline replicates pair inputs over samples.
        explicit = model(
            a.reshape(B * S, N, sc.c_token),
            s.reshape(B * S, N, sc.c_s),
            z.unsqueeze(1).expand(B, S, N, N,
                                  sc.c_z).reshape(B * S, N, N,
                                                  sc.c_z).contiguous(),
...
def real_case():
    ...
    ref = RefProtenixDiffusionModuleFromOSS.build(**_FULL).to(
        device=device, dtype=torch.float32).eval()
    oss_w = {
        k[len("diffusion_module."):]: v
        for k, v in weights.items() if k.startswith("diffusion_module.")
    }
    ...
    with torch.inference_mode():
        exp = ref(x_noisy, t, feats, s_inputs, s_trunk, z_trunk, None, None,
                  None)  # OSS: pair_z / p_lm / c_l = None -> computed fresh
...
def _real_trtbnm_module(sc: Scenario, real_case):
    ...
    converted = convert_diffusion_module_torch(config,
                                               real_case["weights"],
    ...
    batch = {
        k: _bt(feats[k])
        for k in ("atom_to_token_idx", "ref_pos", "ref_charge", "ref_mask",
                  "ref_atom_name_chars", "ref_element", "d_lm", "v_lm", "relp")
    }
...
def test_protenix_diffusion_module(sc: Scenario, real_case):
    model, batch, s_inputs, s_trunk, z_trunk, torch_dtype = _real_trtbnm_module(
        sc, real_case)
    ...
    handle = model.diffusion_transformer.register_forward_pre_hook(
        _capture_token_inputs)
    with torch.inference_mode():
    ...
    assert token_shapes == [
        ((Scenario().n_sample, Scenario().n_token, _FULL["c_token"]),
         (Scenario().n_sample, Scenario().n_token,
          _FULL["c_s"]), (1, Scenario().n_token, Scenario().n_token,
                          _FULL["c_z"]), (1, Scenario().n_token))
    ]

    assert torch.isfinite(act).all(), f"non-finite output ({sc.dtype})"
    r = _rmse_ratio(act, real_case["exp"])
    any_bf16 = "bfloat16" in (sc.dtype, sc.token_dtype, sc.enc_dtype,
                              sc.dec_dtype, sc.z_pair_dtype)
    ...
    assert rc < tol_c, (f"shared-vars cache diverged from uncached: "
                        f"rmse={rc:.3e} exceeds {tol_c:.0e} ({sc.dtype})")
...
def _sampler_inputs(device: torch.device, sc: Scenario, module):
    ...
    feats = {
        ...
        a2t,
        "ref_pos":
        torch.randn(n_atom, 3, device=device),
        "ref_charge":
        torch.zeros(n_atom, device=device),
        "ref_mask":
        torch.ones(n_atom, device=device),
        "ref_element":
        F.one_hot(torch.randint(0, 128, (n_atom, ), device=device),
                  128).float(),
        "ref_atom_name_chars":
        F.one_hot(torch.randint(0, 64, (n_atom, 4), device=device),
                  64).float(),
        "ref_space_uid":
        a2t.clone(),
    }
...
def test_sample_diffusion_shared_vars_cache(sc: Scenario, real_case):
    ...
    torch.backends.cuda.matmul.allow_tf32 = False
    model, batch, s_inputs, s_trunk, z_trunk, _ = _real_trtbnm_module(
        sc, real_case)
    ...
    def _roll(sampler):
        ...
        with torch.inference_mode():
            return sampler.sample_coords(batch,
                                         s_inputs,
                                         s_trunk,
                                         z_trunk,
    ...
    assert r < tol, (f"cache changed the rollout ({sc.dtype}): rmse={r:.3e} "
                     f"max|Δ|={(x_cached - x_ref).abs().max().item():.3e}")
...
def test_sample_coords_drop_consumed_features(real_case):
    ...
    consumed = {
        "relp", "ref_pos", "ref_charge", "ref_mask", "ref_atom_name_chars",
        "ref_element", "d_lm", "v_lm", "pad_info"
    }
    ...
    assert r < 1e-4, (
        f"dropping relp changed the cached rollout: rmse={r:.3e} "
        f"max|Δ|={(x_drop - x_keep).abs().max().item():.3e}")
...
def test_token_transformer_cudagraph_parity(real_case):
    ...
    assert states and all(
        ...
        for ps, fb in states), (
            "expected every captured key GRAPH_VERIFIED with no eager "
            f"fallback, got {[(ps.name, fb) for ps, fb in states]}")
    ...
    assert r < 1e-1, (f"cuda-graph token transformer diverged from eager: "
                      f"rmse={r:.3e} "
