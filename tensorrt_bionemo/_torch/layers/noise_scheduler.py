...
from tensorrt_bionemo._torch.layers.random_augmentation import centre_random_augmentation
...
class SampleDiffusion(nn.Module):
    ...
    def __init__(
        self, *, gamma0: float = 0.8, gamma_min: float = 1.0, noise_scale: float = 1.003, step_scale: float = 1.5
    ) -> None:
        super().__init__()
        self.gamma0 = gamma0
    ...
    def sample(
        ...
        **ctx: Any,
        ...
        """
        x = noise_schedule[0] * torch.randn(coords_shape, device=device, dtype=dtype)
        for sigma_last, c_tau in zip(noise_schedule[:-1], noise_schedule[1:], strict=False):
            ...
            x_noisy = x + self.noise_scale * torch.sqrt(sigma_hat**2 - sigma_last**2) * torch.randn(
