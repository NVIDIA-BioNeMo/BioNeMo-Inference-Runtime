...
def get_conditioned_transition_block_weights(
    ...
    ret = {}
    ret.update(
        get_adaln_weights(state_dict, f"{prefix}.adaln", f"{tbm_prefix}.adaln", dim, dim_single_cond, dtype=dtype)
    )
    ...
    ret.update(
        {
            f"{tbm_prefix}.fused_swl_a_to_b.weight": fused_swl_a_to_b_weight.to(torch_dtype),
            f"{tbm_prefix}.b_to_a.weight": b_to_a_weight.to(torch_dtype),
            f"{tbm_prefix}.output_projection.weight": output_projection_weight.to(torch_dtype),
            f"{tbm_prefix}.output_projection.bias": output_projection_bias.to(torch_dtype),
        }
    )
    return ret
...
def convert_hf_diffusion_transformer(
    ...
    for i in range(config.num_blocks):
        ...
        weights.update(
            get_post_norm_weights(
                tktn_state_dict, f"{layer_prefix}.post_lnorm", f"{layer_tbm_prefix}.post_lnorm", dtype=config.dtype
            )
        )
    ...
    **kwargs,
    ...
    for i in range(config.num_blocks):
        if not config.use_separate_layer_norm:
            tbnm_state_dict[f"layers.{i}.adaln.a_norm"] = [{"weight": torch.ones([dim], dtype=dtype)}]
            ...
            tbnm_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_q.a_norm"] = [
                {"weight": torch.ones([dim], dtype=dtype)}
            ]
            _template_module_weight(
                tbnm_state_dict, module_state_dict, f"layers.{i}.pair_bias_attn.layer_norm_a_q.s_norm"
            )

            tbnm_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_q.fused_s_scale_s_bias"] = [
                {
            ...
            tbnm_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_k.a_norm"] = [
                {"weight": torch.ones([dim], dtype=dtype)}
            ]
            _template_module_weight(
                tbnm_state_dict, module_state_dict, f"layers.{i}.pair_bias_attn.layer_norm_a_k.s_norm"
            )

            tbnm_state_dict[f"layers.{i}.pair_bias_attn.layer_norm_a_k.fused_s_scale_s_bias"] = [
                {
        ...
        tbnm_state_dict[f"layers.{i}.transition.adaln.a_norm"] = [{"weight": torch.ones([dim], dtype=dtype)}]
