...
from test_utils.openfold3.ref_layers_from_oss import \
    RefTemplatePairBlockFromOF3OSS
...
def _assert_pipeline_style_left_aligned_pair_mask(
        mask_bt_nn: torch.Tensor) -> None:
    ...
    assert bool((m[..., :-1] >= m[..., 1:]).all()), (
        "expected left-aligned columns (111...000) per pair row")
    mt = m.transpose(-1, -2).contiguous()
    assert bool((mt[..., :-1] >= mt[..., 1:]).all()), (
        "expected left-aligned rows (111...000) per pair column")
...
def test_template_pair_stack_block(sc: Scenario):
    ...
    t_float = torch.randn(
        bs, sc.n_templ, sc.n_res, sc.n_res, c_in,
    ...
    mask_float = pair_mask_bn[:, None, :, :].expand(bs, sc.n_templ, sc.n_res,
                                                    sc.n_res).contiguous()
    ...
    with torch.no_grad():
        ...
        mask_float = mask_float.flatten(0,
                                        1)  # fused batch_dim and template_dim
        ...
        if test_dtype == torch.float32:
            torch.testing.assert_close(_masked(output_test_dtype),
                                       _masked(ref_output_float),
                                       atol=1e-3,
                                       rtol=1e-4)
            ...
            with torch.amp.autocast(device_type="cuda", dtype=test_dtype):
                ref_output = ref_module(t_float.to(test_dtype),
                                        mask_float.to(test_dtype))

            diff0_max = torch.max(
                torch.abs(
                    _masked(output_test_dtype) - _masked(ref_output_float)))
            diff0_mean = torch.mean(
                torch.abs(
                    _masked(output_test_dtype) - _masked(ref_output_float)))

            diff1_max = torch.max(
                torch.abs(_masked(ref_output) - _masked(ref_output_float)))
            diff1_mean = torch.mean(
                torch.abs(_masked(ref_output) - _masked(ref_output_float)))

            if sc.triangle_attn_backend == "CuTeDSL":
                assert diff0_max <= 2.0 * diff1_max + 1e-3, (
                    f"CuTeDSL bf16 drift vs fp32 ref: diff_ours={diff0_max}, "
                    f"diff_autocast_ref={diff1_max}")
                assert diff0_mean <= 2.0 * diff1_mean + 5e-2, (
                    f"CuTeDSL bf16 mean drift: diff_ours={diff0_mean}, "
                    f"diff_autocast_ref={diff1_mean}")
            else:
                assert (abs(diff0_max - diff1_max) /
                        torch.min(diff0_max, diff1_max) <= 0.5)
