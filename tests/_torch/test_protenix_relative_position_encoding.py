...
def test_protenix_relative_position_encoding(sc: Scenario):
    ...
    ref = RefProtenixRelativePositionEncodingFromOSS.build(
        r_max=sc.r_max, s_max=sc.s_max,
        c_z=sc.c_z).to(device=device, dtype=torch.float32).eval()

    config = RelativePositionEncodingConfig(r_max=sc.r_max,
                                            s_max=sc.s_max,
                                            c_z=sc.c_z,
                                            dtype=sc.dtype)
    model = RelativePositionEncoder(
        token_z=config.c_z,
        r_max=config.r_max,
        s_max=config.s_max,
    ...
    converted = convert_relative_position_encoding_torch(config,
                                                         ref.state_dict(),
                                                         prefix="")
