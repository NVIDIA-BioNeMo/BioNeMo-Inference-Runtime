...
from tensorrt_bionemo._torch.graph_optimization.config import (
    ...
    InputKeyMethod, InputRoutingConfigFactory, NamedDimTies, effective_ranges)
from tensorrt_bionemo._torch.graph_optimization.decorator import (
    bind_forward_args, support_graph_optimization,
...
from tests.common.test_utils.model_forwards import (
    _CKPT_ENV, _HF_CKPT, _model_weights_available)
...
_MODULE_CASES = [
    pytest.param(PairformerModule, ["buffers"], ["mask", "pair_mask"],
                 id="of3_pairformer"),
    pytest.param(OpenFold3DiffusionTransformer, ["buffers"], [],
                 id="of3_token_transformer"),
    pytest.param(OF3DiffusionModule, [], [], id="of3_diffusion_module"),
    pytest.param(BoltzDiffusionTransformer, ["buffers"], [],
                 id="boltz2_token_transformer"),
    pytest.param(BoltzDiffusionModule, [], [], id="boltz2_diffusion_module"),
    pytest.param(ProtenixDiffusionTransformer, ["buffers"], [],
                 id="protenix_token_transformer"),
...
def test_graph_opt_default_declares_num_tokens(cls, workspaces, static_args):
    ...
    assert [(a.name, a.dim_len_max)
            for a in routing.input_acceptance_dims] == [("num_tokens", 1024)]
...
def test_validation_rejects_unknown_name():
    ...
    with pytest.raises(ValueError, match="not a parameter of forward"):
        ...
        @support_graph_optimization(
            named_dims=(NamedDimTies(name="num_tokens",
                                     input_dims=(("does_not_exist", (-2, )), )), ),
...
def test_bind_forward_args_maps_positional_to_names():
    ...
    keyword = bind_forward_args(
        forward, (), {"s": 1, "z": 2, "mask": 3, "pair_mask": 4})
...
def test_export_rejects_acceptance_above_largest_bucket():
    ...
    factory.set_padded_dim("num_tokens", dim_len_min=1, dim_len_max=1024,
                           num_intervals=8, multiple_of=128)
...
# ---------------------------------------------------------------------------
def _decorator_pairformer_config(module_name, input_key_method, sample_ids,
                                 verify_capture=True):
    ...
    return CUDAGraphOptimizationConfig(
        ...
        input_routing_config=H._routing_from_decorator(
            PairformerModule,
            bucket=input_key_method == InputKeyMethod.BUCKETED_SHAPES),
...
@pytest.mark.skipif(not torch.cuda.is_available(),
                    reason="decorator GPU parity requires CUDA")
@pytest.mark.skipif(not H._SAMPLES_AVAILABLE, reason="sample data not found")
@pytest.mark.skipif(
    not _model_weights_available("openfold3", _ckpt_env=_CKPT_ENV,
                                 _hf_ckpt=_HF_CKPT),
    reason="openfold3 checkpoint unavailable")
def test_pairformer_decorator_gpu_parity(monkeypatch):
    monkeypatch.setattr(H, "_openfold3_graph_optimization_config",
                        _decorator_pairformer_config)
    H._assert_cuda_graph_parity(
        "openfold3", "structure_pairformer", sample_ids=_SAMPLE_IDS,
        input_key_method=InputKeyMethod.EXACT)
