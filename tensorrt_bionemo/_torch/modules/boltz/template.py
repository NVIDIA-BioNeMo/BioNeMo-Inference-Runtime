...
class TemplateV2Module(nn.Module):
    ...
    Upstream: ``boltz/model/modules/trunkv2.py::TemplateV2Module``.
    Embeds templates into a ``template_dim`` pair rep, runs a small
    ``PairformerNoSeqModule``, aggregates over T, and projects back to
    ``token_z``. Inner pairformer defaults to bf16 trimul (fused path);
    outer projections use ``config.torch_dtype``.
    """
    ...
    def __init__(self, config: BaseConfig) -> None:
        ...
        self.pairformer_dtype = config.pairformer.torch_dtype
        ...
        self.z_norm = nn.LayerNorm(self.token_z, eps=config.norm_epsilon, dtype=self.dtype)
        self.v_norm = nn.LayerNorm(self.template_dim, eps=config.norm_epsilon, dtype=self.pairformer_dtype)

        self.z_proj = Linear(
            self.token_z, self.template_dim, bias=False, dtype=self.dtype, skip_create_weights=skip_create_weights
        )
        a_in = self.num_tokens * 2 + self.num_bins + 5
        self.a_proj = Linear(
            a_in, self.template_dim, bias=False, dtype=self.dtype, skip_create_weights=skip_create_weights
        )
        self.u_proj = Linear(
            self.template_dim, self.token_z, bias=False, dtype=self.dtype, skip_create_weights=skip_create_weights
        )
        ...
        self.pairformer = PairformerNoSeqModule(
            ...
            trimul_mean_normalization=config.pairformer.trimul_mean_normalization,
        )

        self.register_buffer(
            "boundaries",
            torch.linspace(self.min_dist, self.max_dist, self.num_bins - 1, dtype=torch.float32),
            persistent=False,
        )
    ...
    def load_weights(self, weights: dict):
        ...
        if missing:
            raise ValueError(f"The following weights are not loaded: {missing}")
    ...
    def forward(
        ...
        v = v.to(self.pairformer_dtype)
        v = v + self.pairformer(
            v, pair_mask_t.to(self.pairformer_dtype), attn_metadatas={"triangle_attn": attn_metadata}
        )
        v = self.v_norm(v)
        v = v.view(B, T, N, N, self.template_dim)
