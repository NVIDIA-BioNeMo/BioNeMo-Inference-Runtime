...
from tensorrt_bionemo.confs.modules.transformers import (
    PairformerConfig, TrtPairformerBuildConfig)


def test_pairformer_build_config():
    config = PairformerConfig(architecture="pairformer", dtype="float32")
    build_config = TrtPairformerBuildConfig(
        num_optimization_profiles=4,
        max_seqlen=2040,
        min_seqlen=65,
        align=32,
    ...
    assert len(opt_profiles) == build_config.num_optimization_profiles
