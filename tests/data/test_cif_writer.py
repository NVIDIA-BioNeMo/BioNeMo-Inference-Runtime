...
def _write_outside_the_checkout(tmp_path, monkeypatch):
    ...
    they only assert on the returned string — but :meth:`CIFWriter.write` also
    opens that path for writing, so a run started from the repo root drops a
    ``test.cif`` next to ``setup.py``.
    """
...
class TestCIFWriterMultiPolymer:
    ...
    def test_rna_unknown_nucleotide_is_writable(self, writer):
        """Unknown ribonucleotide (``RX``) writes as component ``N``.

        ``ihm.RNAAlphabet`` holds only A/C/G/U, so this used to raise
        ``KeyError: 'N'`` from inside ``modelcif.Entity``.
        """
        out = writer.write(rna_only_folding(sequence="AGN", with_residue_names=True, with_mol_types=True))
        assert "UNKNOWN RIBONUCLEOTIDE" in out, "the N component should be declared in chem_comp"
        assert "AGN" in out, "entity_poly should carry the unknown base in the sequence"
    ...
    def test_dna_unknown_nucleotide_is_writable(self, writer):
        """Unknown deoxyribonucleotide (``DX``) writes as ``DN``.

        The canonical one-letter column still reads ``N``, as ihm spells
        ``DA`` with canonical ``A``.
        """
        out = writer.write(dna_only_folding(sequence="ACN", with_residue_names=True, with_mol_types=True))
        assert "UNKNOWN DEOXYRIBONUCLEOTIDE" in out, "the DN component should be declared in chem_comp"
        assert "(DA)(DC)(DN)" in out, "entity_poly should spell the unknown base as DN"
        assert any(l.startswith("ATOM") for l in out.split("\n"))
        assert not any(l.startswith("HETATM") for l in out.split("\n"))

    def test_protein_asparagine_is_not_unknown_rna(self, writer):
        """Protein restype ``N`` is Asn, not the unknown-RNA ChemComp ``N``."""
        out = writer.write(protein_only_folding(sequence="AN", with_mol_types=True))
        assert "UNKNOWN RIBONUCLEOTIDE" not in out
        assert "ASN" in out
