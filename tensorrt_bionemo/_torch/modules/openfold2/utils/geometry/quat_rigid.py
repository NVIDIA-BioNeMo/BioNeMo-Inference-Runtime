...
from tensorrt_bionemo._torch.modules.openfold2.utils.geometry.rotation_matrix import Rot3Array
from tensorrt_bionemo._torch.modules.openfold2.utils.geometry.vector import Vec3Array


class QuatRigid(nn.Module):
    def __init__(self, c_hidden: int, dtype: torch.dtype = torch.float32, skip_create_weights: bool = False):

        super().__init__()

        self.linear = Linear(c_hidden, 6, bias=True, dtype=dtype, skip_create_weights=skip_create_weights)
