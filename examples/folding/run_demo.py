...
DEFAULT_INPUT = (Path(__file__).resolve().parents[1] / "data" / "samples" /
                 "monomers" / "T1031.json")
        ...
        for polymer in entry.get("polymers", []):
            polymers.append(
                Polymer(
                    ...
                    paired_msas=_load_msas(polymer.get("paired_msas"),
                                           path.parent),
                    templates=_load_templates(polymer.get("templates"),
                                              path.parent),
                ))
        requests.append(
            InputRequest(input_id=entry["input_id"], polymers=polymers))
    return requests
...
def _engine_kwargs(model_source: str) -> dict[str, Any]:
    ...
    config.diffusion_module_config.atom_transformer_encoder_config.\
        shared_pair_norm = False
    config.diffusion_module_config.atom_transformer_decoder_config.\
        shared_pair_norm = False
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run a folding model through TRT-BioNeMo build_processor.")
    ...
    failed = [
        ...
        if (row.get("__inference_error__") or {}).get("error_msg")
    ]
    if failed:
        raise RuntimeError(
            f"{len(failed)} of {len(outputs)} predictions failed: "
            f"{[row.get('__record_id') for row in failed]}")
