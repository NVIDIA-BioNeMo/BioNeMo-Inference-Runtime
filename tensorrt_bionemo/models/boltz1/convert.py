...
def get_transition_weights(state_dict: dict, prefix: str, tbm_prefix: str, dim: int = 128, dtype: str = "float32"):
    torch_dtype = str_dtype_to_torch(dtype)
    ...
    ret = {
        ...
        f"{tbm_prefix}.fused_fc2_fc1.weight": fused_fc2_fc1_weight.to(torch_dtype),
...
def convert_hf_pairformer(
    config: BaseConfig, pairformer_type: str = "structure", local_checkpoint: str = None, model_name: str = "boltz-1"
):
    """
    ...
    logger.info(f"Loading weights for {pairformer_type} pairformer, dtype: {config.dtype}")
    for i in range(config.num_blocks):
        ...
        weights.update(
            get_tri_attn_node_weights(
                state_dict, f"{layer_prefix}.tri_att_start", f"{layer_tbm_prefix}.tri_attn_start", dtype=config.dtype
            )
        )
        weights.update(
            get_tri_attn_node_weights(
                state_dict, f"{layer_prefix}.tri_att_end", f"{layer_tbm_prefix}.tri_attn_end", dtype=config.dtype
            )
        )
        weights.update(
            get_tri_mul_node_weights(
                state_dict, f"{layer_prefix}.tri_mul_out", f"{layer_tbm_prefix}.tri_mul_out", dtype=config.dtype
            )
        )
        weights.update(
            get_tri_mul_node_weights(
                state_dict, f"{layer_prefix}.tri_mul_in", f"{layer_tbm_prefix}.tri_mul_in", dtype=config.dtype
            )
        )
        if not config.no_update_s:
            weights.update(
...
def get_adaln_weights(
    state_dict: dict, prefix: str, tbm_prefix: str, dim: int, dim_single_cond: int, dtype: str = "float32"
):
    torch_dtype = str_dtype_to_torch(dtype)

    s_norm_weight = state_dict[f"{prefix}.s_norm.weight"]
    a_norm_weight = torch.ones([dim], dtype=torch_dtype, device=s_norm_weight.device)
    ...
    s_bias_bias = torch.zeros([dim], dtype=torch_dtype, device=s_scale_bias.device)

    fused_s_scale_s_bias_weights = torch.cat([s_scale_weight, s_bias_weight], dim=0)
    fused_s_scale_s_bias_bias = torch.cat([s_scale_bias, s_bias_bias], dim=0)

    ret = {
        f"{tbm_prefix}.a_norm.weight": a_norm_weight.to(torch_dtype),
        f"{tbm_prefix}.s_norm.weight": s_norm_weight.to(torch_dtype),
        f"{tbm_prefix}.fused_s_scale_s_bias.weight": fused_s_scale_s_bias_weights.to(torch_dtype),
        f"{tbm_prefix}.fused_s_scale_s_bias.bias": fused_s_scale_s_bias_bias.to(torch_dtype),
    }
    return ret
...
def get_output_projection_weights(state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32"):
    torch_dtype = str_dtype_to_torch(dtype)
    ret = {}
...
def get_post_norm_weights(state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32"):
    torch_dtype = str_dtype_to_torch(dtype)
    ret = {}
...
def convert_hf_diffusion_transformer(
    config: BaseConfig = None, local_checkpoint: str = None, model_name: str = "boltz-1"
):
    """
    ...
    for i in range(config.num_blocks):
        ...
        weights.update(
            get_post_norm_weights(
                state_dict, f"{layer_prefix}.post_lnorm", f"{layer_tbm_prefix}.post_lnorm", dtype=config.dtype
            )
        )
...
def convert_hf_diffusion_transformer_torch(
    ...
    for i in range(config.num_blocks):
        ...
        tbnm_state_dict[f"layers.{i}.adaln.s_norm"] = [{"weight": module_state_dict[f"layers.{i}.adaln.s_norm.weight"]}]
...
def convert_hf_msa_module_torch(
    ...
    tbnm_state_dict = {}
    tbnm_state_dict["s_proj"] = [
        {
            "weight": module_state_dict["s_proj.weight"],
        }
    ]
    tbnm_state_dict["msa_proj"] = [
        {
    ...
    for i in range(config.msa_blocks):
        ...
        tbnm_state_dict[f"layers.{i}.pair_weighted_averaging.fused_proj_m_g"] = [
            {
        ...
        # weight for tri_attn_start and tri_attn_end
        for name in ["start", "end"]:
            tbnm_state_dict[f"layers.{i}.pairformer_layer.tri_attn_{name}.layer_norm"] = [
                {
                    "weight": module_state_dict[f"layers.{i}.tri_att_{name}.layer_norm.weight"],
                    "bias": module_state_dict[f"layers.{i}.tri_att_{name}.layer_norm.bias"],
                }
            ]
            tbnm_state_dict[f"layers.{i}.pairformer_layer.tri_attn_{name}.linear"] = [
                {
                    "weight": module_state_dict[f"layers.{i}.tri_att_{name}.linear.weight"],
                }
            ]
            tbnm_state_dict[f"layers.{i}.pairformer_layer.tri_attn_{name}.mha.qkv_proj"] = [
                {
                    "weight": module_state_dict[f"layers.{i}.tri_att_{name}.mha.linear_q.weight"],
                },
                {
            ...
            tbnm_state_dict[f"layers.{i}.pairformer_layer.tri_attn_{name}.mha.o_proj"] = [
                {
                    "weight": module_state_dict[f"layers.{i}.tri_att_{name}.mha.linear_o.weight"],
                }
            ]
            tbnm_state_dict[f"layers.{i}.pairformer_layer.tri_attn_{name}.mha.g_proj"] = [
                {
                    "weight": module_state_dict[f"layers.{i}.tri_att_{name}.mha.linear_g.weight"],
                }
            ]
        ...
        tbnm_state_dict[f"layers.{i}.pairformer_layer.transition_z.fused_fc2_fc1"] = [
            {
...
def convert_hf_diffusion_conditioning_torch(
    ...
    for i in range(num_transitions):
        ...
        tbnm_state_dict[f"pairwise_conditioner.transitions.{i}.fused_fc2_fc1"] = [
            {
