...
from .utils.feats import (build_template_angle_feat, build_template_pair_feat,
                          dgram_from_positions, pseudo_beta_fn)


def relpos(ri: torch.Tensor, boundaries: torch.Tensor,
           linear_relpos: Linear) -> torch.Tensor:
    """
...
class InputEmbedderMultimer(nn.Module):
    ...
    def relpos(self, residue_index: torch.Tensor, asym_id: torch.Tensor,
               entity_id: torch.Tensor, sym_id: torch.Tensor) -> torch.Tensor:
        """
        Computes relative positional encodings
        Args:
            residue_index:
...
class RecyclingEmbedder(nn.Module):
    ...
    def __init__(self, config: BaseConfig):
        ...
        self.linear = Linear(self.no_bins,
                             self.c_z,
                             bias=True,
                             dtype=config.torch_dtype,
                             mapping=config.mapping,
...
class TemplateEmbedder(nn.Module):
    ...
    def __init__(self, config: BaseConfig):
        ...
        self.template_pair_stack = TemplatePairStack(
            ...
            triangle_attn_node_chunk_size=mc.triangle_attn_node_chunk_size,
            dtype=mc.torch_dtype,
            mapping=mc.mapping,
            skip_create_weights=mc.skip_create_weights,
        )
    ...
    def forward(
        ...
        for i in range(n_templ):
            ...
            single_template_feats = tensor_tree_map(
                lambda t: torch.index_select(t, templ_dim, idx).squeeze(
                    templ_dim),
                batch,
            )
...
class TemplatePairEmbedderMultimer(nn.Module):
    ...
    def forward(
        ...
        backbone_mask_2d = (backbone_mask[..., None] *
                            backbone_mask[..., None, :])
        backbone_mask_2d *= multichain_mask_2d
        x, y, z = [(coord * backbone_mask_2d).to(dtype=query_embedding.dtype)
