...
def atom14_to_atom37(
    ...
    idx_atom37_to_atom14 = get_rc_tensor(rc.RESTYPE_ATOM37_TO_ATOM14,
                                         aatype).long()
...
def get_alt_atom14(aatype, positions: torch.Tensor, mask):
    ...
    alternative_positions = torch.sum(positions[..., None, :] *
...
def torsion_angles_to_frames(
    ...
    all_rots = geometry.Rot3Array(ones, zeros, zeros, zeros, cos_angles,
                                  -sin_angles, zeros, sin_angles, cos_angles)
    ...
    all_frames_to_backb = Rigid3Array.cat([
        all_frames[..., 0:5], chi2_frame_to_backb[..., None],
        chi3_frame_to_backb[..., None], chi4_frame_to_backb[..., None]
    ],
                                          dim=-1)
...
def frames_and_literature_positions_to_atom14_pos(
    ...
    residx_to_group_idx = get_rc_tensor(rc.restype_atom14_to_rigid_group,
                                        aatype)
...
def extreme_ca_ca_distance_violations(
    ...
    has_no_gap_mask = ((residue_index[..., 1:] -
                        residue_index[..., :-1]) == 1.0).astype(
                            positions.x.dtype)
...
def compute_chi_angles(positions: geometry.Vec3Array, mask: torch.Tensor,
                       aatype: torch.Tensor):
    ...
    chi_angle_atoms = positions.map_tensor_fn(
        partial(tensor_utils.batched_gather,
                inds=atom_indices,
                dim=-1,
                no_batch_dims=no_batch_dims + 1))
...
def make_transform_from_reference(
        a_xyz: geometry.Vec3Array, b_xyz: geometry.Vec3Array,
