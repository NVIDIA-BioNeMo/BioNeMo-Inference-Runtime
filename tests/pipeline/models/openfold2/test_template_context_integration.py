...
def _template(chain_id: str = "A") -> TemplateParsed:
    return TemplateParsed(content=_TEMPLATE_CIF.read_text(), format="cif", chain_id=chain_id)


def test_monomer_context_uses_supplied_template():
    parsed = InputParsed(
        input_id="monomer_template",
        polymers=[PolymerParsed(chain_id="Q", sequence="MR", msas=None, templates=[_template()])],
    )
    ...
    parsed = InputParsed(
        ...
        polymers=[
            PolymerParsed(
                ...
                sequence="MR",
                msas=None,
                templates=[TemplateParsed(content="not a CIF", chain_id="missing")],
            )
        ],
    )
...
def test_heteromer_context_keeps_template_on_its_polymer():
    parsed = InputParsed(
        ...
        polymers=[
            PolymerParsed(chain_id=["Q"], sequence="MR", msas=None, paired_msas=None, templates=[_template("A")]),
            PolymerParsed(chain_id=["R"], sequence="AA", msas=None, paired_msas=None, templates=None),
        ],
    )
...
def test_homomer_context_reuses_entity_template_for_each_chain():
    parsed = InputParsed(
        ...
        polymers=[
            PolymerParsed(chain_id=["Q", "R"], sequence="MR", msas=None, paired_msas=None, templates=[_template("A")])
        ],
    )
...
def test_multimer_uses_configured_template_limit():
    ...
    result = FeatureContextGenerator(MockConfig(is_multimer=True, max_templates=2))(parsed)
...
def test_negative_template_limit_is_rejected(enable_template):
    with pytest.raises(ValueError, match="max_templates.*non-negative"):
        FeatureContextGenerator(MockConfig(enable_template=enable_template, max_templates=-1))
...
def test_multimer_context_allows_zero_template_limit():
    ...
    result = FeatureContextGenerator(MockConfig(is_multimer=True, max_templates=0))(parsed)
...
def test_real_stage_specs_transform_and_derive_template_features():
    ...
    config = OpenFold2_FT2_Config(max_recycling_iters=0, max_msa_clusters=2, max_extra_msa=1)
    ...
    tokenized = asyncio.run(tokenizer_udf.udf_for_item({"parsed": parsed, "__record_id": "real_stage_specs"}))
    ...
    assert features["template_pseudo_beta"].shape == (config.max_templates, 2, 3, 1)
    assert features["template_torsion_angles_sin_cos"].shape == (config.max_templates, 2, 7, 2, 1)
    assert features["template_pseudo_beta_mask"][0, :, 0].count_nonzero() == 2
    assert features["template_torsion_angles_mask"][0, :, :, 0].count_nonzero() > 0
...
def test_real_multimer_stage_specs_preserve_template_atom_mask(with_template):
    ...
    assert features["template_all_atom_mask"].shape == (config.max_templates, 4, 37, 1)
    assert bool(features["template_all_atom_mask"].count_nonzero()) is with_template
