...
def load_pair_transition_weights_torch(module, weights_and_biases, dtype=torch.float32):
    ...
    module.fused_fc2_fc1.weight.data.copy_(
        torch.cat([linear_b_weight.to(dtype).to("cuda"), linear_a_weight.to(dtype).to("cuda")], dim=0)
    )
...
def load_msa_pair_weighted_averaging_weights_torch(module, weights_and_biases, dtype=torch.float32):
