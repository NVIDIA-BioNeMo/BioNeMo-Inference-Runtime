...
class Openfold3RefPairwiseSelfAttention(BoltzRefPairwiseSelfAttention):
    ...
    def load_weights(
        ...
        for key in state_dict.keys():
            if layer_path + f".{key_layer}" in key:
                if "mha" in key:
                ...
                if "layer_norm_z" in key:
                    name = key.replace(key_layer + ".layer_norm_z", "pair_bias_attn.proj_z.0")
                    extract_state_dict[name] = state_dict[key]

                if "linear_z" in key:
                    name = key.replace(key_layer + ".linear_z", "pair_bias_attn.proj_z.1")
                    extract_state_dict[name] = state_dict[key]
        ...
        if f"{layer_path}.norm_s.weight" in extract_state_dict:
            ...
            attn.proj_z[0] = nn.LayerNorm(c_z, bias=False)
            layers = [attn.proj_q, attn.proj_k, attn.proj_v, attn.proj_g, attn.proj_z[0], attn.proj_z[1], attn.proj_o]
...
class Openfold3RefConditionedTransitionBlock(BoltzRefConditionedTransitionBlock):
    ...
    def load_weights(
        ...
        adaln = Openfold3RefAdaLN.load_weights(
            state_dict=state_dict, layer_path=layer_path, key_layer="conditioned_transition.layer_norm"
        )
        ...
        for key in state_dict.keys():
            if layer_path + f".{key_layer}" in key:
                ...
                if "swiglu" in key:
                    name = key.replace(key_layer + ".swiglu.linear_a.weight", "swish_gate.0_a.weight")
                    name = name.replace(key_layer + ".swiglu.linear_b.weight", "swish_gate.0_b.weight")
                    extract_state_dict[name] = state_dict[key]

                if "linear_out" in key or "linear_g" in key:
                    name = key.replace(key_layer + ".linear_out", "b_to_a")
                    name = name.replace(key_layer + ".linear_g", "output_projection.0")
                    extract_state_dict[name] = state_dict[key]
...
class Openfold3RefDiffusionTransformerLayer(BoltzRefDiffusionTransformerLayer):
    ...
    def load_weights(
        ...
        adaln = Openfold3RefAdaLN.load_weights(
            state_dict=state_dict, layer_path=layer_path, key_layer="attention_pair_bias.layer_norm_a"
        )
        pair_bias_attn = Openfold3RefPairwiseSelfAttention.load_weights(
            state_dict=state_dict, layer_path=layer_path, key_layer="attention_pair_bias"
        )

        transition = Openfold3RefConditionedTransitionBlock.load_weights(
            state_dict=state_dict, layer_path=layer_path, key_layer="conditioned_transition"
        )
        ...
        for key in state_dict.keys():
            if layer_path + f".{key_layer}" in key:
                if "linear_ada_out" in key:
                    name = key.replace(key_layer + ".linear_ada_out", "output_projection.0")
                    extract_state_dict[name] = state_dict[key]
        assert extract_state_dict != {}, "extract weights is empty"
        weights_biases_path = [
            (f"{layer_path}.output_projection.0.weight", f"{layer_path}.output_projection.0.bias"),
