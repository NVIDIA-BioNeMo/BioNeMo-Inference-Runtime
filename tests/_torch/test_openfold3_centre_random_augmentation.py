...
The EDM rollout builds atom positions with an extra diffusion-samples axis ``S``
— ``xl = [B, S, N_atom, 3]`` — while ``atom_mask`` stays ``[B, N_atom]``
...
from bionemo_ir._torch.layers.random_augmentation import broadcast_atom_mask, centre_random_augmentation
...
def test_centre_random_augmentation_centers_per_batch_sample(B, S, N):
    ...
    out = centre_random_augmentation(xl, atom_mask, s_trans=0.0, generator=gen)
