...
from test_utils.boltz.create_and_load_weights import (
    ...
    load_pairformer_layer_weights_torch,
)
...
def test_pairformer_layer(sc: Scenario):
    ...
    pairwise_metadata_cls = get_attention_backend(sc.pairwise_attn_backend, AttentionType.PAIRWISE).Metadata
    ...
    mask = make_left_aligned_mask(bs, sc.seq_len, dtype=torch.float32, device=device, min_valid=sc.seq_len // 2)
    ...
    with torch.inference_mode():
        ...
        for name, module in ref_layer.named_modules():
            if name.startswith(("tri_attn_start.", "tri_attn_end.", "transition_s.", "transition_z.", "attention.")):
                module.to(dtype)
            elif name.startswith(("tri_mul_out.", "tri_mul_in.")):
                if "norm_out" not in name and "p_out" not in name and "g_out" not in name:
                    module.to(dtype)
                else:
                    module.float()
    ...
    if dtype == torch.float32:
        ...
        torch.testing.assert_close(_masked(ref_s, s_keep), _masked(output_s, s_keep), atol=4e-2, rtol=4e-2)
        torch.testing.assert_close(_masked(ref_z, z_keep), _masked(output_z, z_keep), atol=4e-2, rtol=4e-2)
    else:
        ...
        for name, out_v, ref_v, ref_f32_v, keep in [
            ...
            diff_ours = torch.max(torch.abs(_masked(out_v, keep) - _masked(ref_f32_v, keep)))
            diff_ref = torch.max(torch.abs(_masked(ref_v, keep) - _masked(ref_f32_v, keep)))
            assert diff_ours <= tol_mult * diff_ref + 1e-3, (
                f"{name}: ours_diff={diff_ours.item()}, ref_diff={diff_ref.item()}"
            )
...
def test_precompute_pair_masks(backend_name: str, dtype: torch.dtype):
    ...
    precomputed = precompute_pair_masks(backend_name, pair_mask, inf=inf_val, dtype=dtype)
    ...
    expected_bias_t = (inf_val * (mask_typed.transpose(-2, -1) - 1))[..., :, None, None, :]
    torch.testing.assert_close(precomputed.mask_bias, expected_bias)
    torch.testing.assert_close(precomputed.mask_bias_transposed, expected_bias_t)


@pytest.mark.parametrize("dtype", [torch.float32, torch.bfloat16])
@pytest.mark.parametrize("seq_len", [16, 13])
def test_precompute_pair_masks_cutedsl_left_aligned(dtype: torch.dtype, seq_len: int):
    ...
    n_valid = torch.tensor([seq_len, max(1, seq_len - 3)], dtype=torch.int64, device=device)
    seq_mask = (torch.arange(seq_len, device=device).view(1, seq_len) < n_valid.view(B, 1)).to(torch.float32)
    pair_mask = seq_mask[..., None] * seq_mask[..., None, :]

    precomputed = precompute_pair_masks("CuTeDSL", pair_mask, inf=inf_val, dtype=dtype)
    ...
    expected = torch.where(
        seq_mask.bool(), n_valid.view(B, 1).to(torch.int32), torch.zeros_like(seq_mask, dtype=torch.int32)
    )
...
def test_pairformer_layer_precomputed_masks(sc: Scenario):
    ...
    s = torch.randn(bs, sc.seq_len, ref_layer.token_s, dtype=dtype, device=device)
    z = torch.randn(bs, sc.seq_len, sc.seq_len, ref_layer.token_z, dtype=dtype, device=device)
...
def _make_minimal_pairformer_layer(
    triangle_attn_backend: str = "VANILLA", dtype: torch.dtype = torch.bfloat16
) -> PairformerLayerV1:
...
def test_get_dual_gemm_x_x_op_pair_mask_left_aligned_flag():
    ...
    op_bipartite = get_dual_gemm_x_x_op(torch.bfloat16, transpose_out=False, N=128, K=128, pair_mask_left_aligned=False)
    assert op_bipartite is not _invoke_cute_dual_gemm_x_x, (
        f"pair_mask_left_aligned=False must route around the CuTe LM kernel, got {op_bipartite.__name__}"
    )

    op_transpose = get_dual_gemm_x_x_op(torch.bfloat16, transpose_out=True, N=128, K=128, pair_mask_left_aligned=False)
