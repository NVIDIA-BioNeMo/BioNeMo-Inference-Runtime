...
def get_pairwise_attn_weights(
    ...
    ret = {
        f"{bioir_prefix}.proj_q.weight": q_weight.to(torch_dtype),
        f"{bioir_prefix}.proj_q.bias": q_bias.to(torch_dtype),
        f"{bioir_prefix}.proj_kv.weight": kv_weights.to(torch_dtype),
        f"{bioir_prefix}.proj_g.weight": g_weight.to(torch_dtype),
        f"{bioir_prefix}.proj_o.weight": o_weight.to(torch_dtype),
    }
    if attention_initial_norm:
        ...
        ret[f"{bioir_prefix}.norm_s.weight"] = init_norm_weight.to(torch_dtype)
        ret[f"{bioir_prefix}.norm_s.bias"] = init_norm_bias.to(torch_dtype)
    ...
    if compute_pair_bias:
        ...
        ret.update(
            {
                f"{bioir_prefix}.proj_z_norm.weight": norm_z_weight.to(torch_dtype),
                f"{bioir_prefix}.proj_z.weight": z_weight.to(torch_dtype),
            }
        )
        if norm_z_bias is not None:
            ret.update(
                {
                    f"{bioir_prefix}.proj_z_norm.bias": norm_z_bias.to(torch_dtype),
                }
            )
    return ret
...
def get_adaln_weights(
    state_dict: dict, prefix: str, bioir_prefix: str, dim: int, dim_single_cond: int, dtype: str = "float32"
):
    torch_dtype = str_dtype_to_torch(dtype)
    ...
    ret = {
        f"{bioir_prefix}.a_norm.weight": a_norm_weight.to(torch_dtype),
        f"{bioir_prefix}.s_norm.weight": s_norm_weight.to(torch_dtype),
        f"{bioir_prefix}.fused_s_scale_s_bias.weight": fused_s_scale_s_bias_weights.to(torch_dtype),
        f"{bioir_prefix}.fused_s_scale_s_bias.bias": fused_s_scale_s_bias_bias.to(torch_dtype),
    }
    return ret
...
def get_output_projection_weights(state_dict: dict, prefix: str, bioir_prefix: str, dtype: str = "float32"):
    torch_dtype = str_dtype_to_torch(dtype)
    ret = {}
    ...
    ret.update(
        {
            f"{bioir_prefix}.weight": output_projection_weight.to(torch_dtype),
            f"{bioir_prefix}.bias": output_projection_bias.to(torch_dtype),
        }
    )
    return ret


def get_post_norm_weights(state_dict: dict, prefix: str, bioir_prefix: str, dtype: str = "float32"):
    torch_dtype = str_dtype_to_torch(dtype)
    ret = {}
    ...
    if weight_key in state_dict and bias_key in state_dict:
        return {
            f"{bioir_prefix}.weight": state_dict[weight_key].to(torch_dtype),
            f"{bioir_prefix}.bias": state_dict[bias_key].to(torch_dtype),
        }
    return ret
    ...
    for i in range(config.num_blocks):
        ...
        weights.update(
            get_post_norm_weights(
                state_dict, f"{layer_prefix}.post_lnorm", f"{layer_bioir_prefix}.post_lnorm", dtype=config.dtype
            )
        )
