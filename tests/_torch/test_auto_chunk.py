...
# isort: off
from bionemo_ir._torch.utils.auto_chunk import (
...
def _force(chunk_size, dim=0, min_rank=0):
    ...
    return ChunkPolicy(chunk_size=chunk_size, min_size=0, dim=dim, min_rank=min_rank)
...
def test_policy_resolved_min_size_auto_delegates_to_default():
    ...
    assert ChunkPolicy(min_size=AUTOCHUNK_MIN_AUTO).resolved_min_size() == default_autochunk_min()
...
def test_registry_enable_disable():
    reg = ChunkRegistry()
    reg.register("a", ChunkPolicy(chunk_size=9, min_size=5, dim=2, enabled=True))
...
def test_default_autochunk_min_scaling(total_gb, expected, autochunk_cache_guard):
    assert _autochunk_min_for_gpu(total_gb) == expected


@pytest.mark.parametrize("total_gb", [8.0, 16.0, 24.0, 48.0, 141.0, 640.0])
def test_default_autochunk_min_is_floored_128_multiple(total_gb, autochunk_cache_guard):
    val = _autochunk_min_for_gpu(total_gb)
    assert val >= _AUTOCHUNK_MIN_FLOOR
    assert val % 128 == 0


def test_default_autochunk_min_no_cuda_returns_reference(autochunk_cache_guard):
    default_autochunk_min.cache_clear()
...
def test_default_autochunk_min_falls_back_on_error(autochunk_cache_guard):
    ...
    with (
        mock.patch("torch.cuda.is_available", return_value=True),
        mock.patch("torch.cuda.current_device", return_value=0),
        mock.patch("torch.cuda.get_device_properties", side_effect=RuntimeError("no device")),
    ):
        assert default_autochunk_min() == DEFAULT_AUTOCHUNK_MIN_REF
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"min_size": 100},  # below threshold
        {"chunk_size": 0},  # chunking disabled
    ],
)
...
def test_chunk_apply_kwargs_are_passed_through():
    x = torch.arange(20, dtype=torch.float32).reshape(10, 2)
    out = chunk_apply(lambda t, *, scale: t * scale, x, policy=_force(3), scale=5)
