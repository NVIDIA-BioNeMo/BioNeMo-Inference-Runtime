...
from tensorrt_bionemo.pipeline.models.openfold2.template_logic import (
    ChainTemplateData, align_query_to_template_chain, build_template_feats,
    extract_template_chains, select_template_for_cif, stable_top_k)
...
def test_alignment_keeps_only_mutually_aligned_positions(monkeypatch):
    monkeypatch.setattr(template_logic, "run_kalign",
                        lambda sequences: ["AC-D", "A-CD"])
    ...
    idx_map, sequence_identity, coverage = align_query_to_template_chain(
        "ACD", chain)

    np.testing.assert_array_equal(idx_map,
                                  np.asarray([[1, 1], [3, 3]], dtype=np.int64))
...
def test_extract_uses_author_chain_ids_and_corrects_atoms(cif_content):
    ...
    all_coordinates = np.stack([
        coordinate for atoms in chains["A"].coords_by_pos.values()
        for coordinate in atoms.values()
    ])
    np.testing.assert_allclose(all_coordinates.mean(axis=0),
                               np.zeros(3),
                               atol=1e-6,
                               rtol=0)
...
def test_coordinate_centering_uses_canonical_atom37_order():
    ...
    canonical_center = np.stack(
        [coordinates[1]["N"], coordinates[1]["CA"],
         coordinates[1]["C"]]).mean(axis=0)
...
def test_explicit_author_chain_has_no_score_rejection(cif_content):
    selected = select_template_for_cif("VV",
                                       cif_content,
                                       specified_chain_id="B")
...
def test_fewer_than_five_aligned_atoms_are_rejected(cif_content):
    sparse = "\n".join(
        line for line in cif_content.splitlines()
        if not ((line.startswith("ATOM") or line.startswith("HETATM"))
                and " L1 " in line and not line.startswith("ATOM   1  ")))
...
def test_stable_top_k_sorts_scores_and_preserves_ties():
    items = [("low", 0.1), ("first-high", 0.8), ("second-high", 0.8),
             ("middle", 0.5)]
...
def test_build_template_feats_packs_atom37_and_openfold_metadata(cif_content):
    ...
    assert all(
        features[key].dtype == np.float32 for key in {
    ...
    np.testing.assert_array_equal(
        ...
        np.asarray([[rc.HHBLITS_AA_TO_ID["M"], rc.HHBLITS_AA_TO_ID["R"]]]))
    np.testing.assert_array_equal(features["template_aatype"].sum(axis=-1),
                                  np.ones((1, 2), dtype=np.int64))
    np.testing.assert_array_equal(features["template_sum_probs"],
                                  np.ones((1, 1), dtype=np.float32))
    np.testing.assert_array_equal(
        features["template_domain_names"],
        np.asarray([b"mini_template_A"], dtype=object))
    np.testing.assert_array_equal(features["template_sequence"],
                                  np.asarray([b"MR"], dtype=object))
    ...
    np.testing.assert_array_equal(positions[~mask],
                                  np.zeros_like(positions[~mask]))
    np.testing.assert_allclose(positions[mask].mean(axis=0),
                               np.zeros(3),
                               atol=1e-6,
                               rtol=0)

    arg = positions[0, 1]
    nh1_distance = np.linalg.norm(arg[rc.atom_order["NH1"]] -
                                  arg[rc.atom_order["CD"]])
    nh2_distance = np.linalg.norm(arg[rc.atom_order["NH2"]] -
                                  arg[rc.atom_order["CD"]])
    assert nh1_distance < nh2_distance
...
def test_build_template_feats_emits_hhblits_gap_class(cif_content,
                                                      monkeypatch):
    monkeypatch.setattr(template_logic, "run_kalign",
                        lambda sequences: ["MQR", "M-R"])

    features = build_template_feats("MQR", [{
        "content": cif_content,
        "chain_id": "A"
    }])
    ...
    np.testing.assert_array_equal(
        features["template_aatype"].argmax(axis=-1),
        np.asarray([[
            rc.HHBLITS_AA_TO_ID["M"], rc.HHBLITS_AA_TO_ID["-"],
            rc.HHBLITS_AA_TO_ID["R"]
        ]]))
    np.testing.assert_array_equal(features["template_sequence"],
                                  np.asarray([b"M-R"], dtype=object))
...
def test_build_template_feats_ranks_by_score_before_top_k(cif_content):
    ...
    np.testing.assert_array_equal(
        features["template_domain_names"],
        np.asarray([b"mini_template_A", b"mini_template_B"], dtype=object))
    np.testing.assert_array_equal(features["template_sum_probs"],
                                  np.ones((2, 1), dtype=np.float32))


def test_build_template_feats_preserves_input_order_for_score_ties(
        cif_content):
    contents = [
        cif_content.replace("data_MINI_TEMPLATE", f"data_TIE_{index}")
    ...
    np.testing.assert_array_equal(
        features["template_domain_names"],
        np.asarray([b"tie_0_A", b"tie_1_A"], dtype=object))
...
def test_empty_templates_match_existing_no_template_shapes():
    ...
    np.testing.assert_array_equal(features["template_domain_names"],
...
def test_invalid_supplied_template_reports_its_input_index(cif_content):
    with pytest.raises(ValueError,
                       match="Invalid supplied template at index 0"):
        build_template_feats("MR", [{"content": cif_content, "format": "pdb"}])


def test_build_template_feats_warns_on_excess_input_templates(
        cif_content, caplog):
