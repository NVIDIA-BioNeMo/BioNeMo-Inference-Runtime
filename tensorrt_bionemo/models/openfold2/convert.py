...
def get_linear_weights(state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32"):
...
def get_outer_product_mean_weights(state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32"):
    torch_dtype = str_dtype_to_torch(dtype)
    ret = {}
    ...
    ret[f"{tbm_prefix}.fused_proj_a_b.weight"] = fused_proj_a_b_weight.to(torch_dtype)
    if proj_a_bias is not None and proj_b_bias is not None:
        fused_proj_a_b_bias = torch.cat([proj_a_bias, proj_b_bias], dim=0)
        ret[f"{tbm_prefix}.fused_proj_a_b.bias"] = fused_proj_a_b_bias.to(torch_dtype)
...
def get_msa_attention_weights(
    state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32", pair_bias: bool = True
):
    torch_dtype = str_dtype_to_torch(dtype)
    ret = {}
    ...
    ret[f"{tbm_prefix}.layer_norm_m.weight"] = layer_norm_m_weight.to(torch_dtype)
    ret[f"{tbm_prefix}.layer_norm_m.bias"] = layer_norm_m_bias.to(torch_dtype)

    if pair_bias:
        ret[f"{tbm_prefix}.proj_z_norm.weight"] = layer_norm_z_weight.to(torch_dtype)
        ret[f"{tbm_prefix}.proj_z_norm.bias"] = layer_norm_z_bias.to(torch_dtype)
...
def get_msa_transition_weights(state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32"):
    torch_dtype = str_dtype_to_torch(dtype)
    ret = {}
...
def get_tri_mul_node_weights(state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32"):
    torch_dtype = str_dtype_to_torch(dtype)
    ret = {}
...
def get_tri_attn_node_weights(state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32"):
    torch_dtype = str_dtype_to_torch(dtype)
    layer_norm_weight = state_dict[f"{prefix}.layer_norm.weight"]
    layer_norm_bias = state_dict[f"{prefix}.layer_norm.bias"]
...
def get_pair_transition_weights(state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32"):
    torch_dtype = str_dtype_to_torch(dtype)
    ret = {}
...
def get_trimul_torch_weights(state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32"):
...
def get_triattn_torch_weights(state_dict: dict, prefix: str, tbm_prefix: str, dtype: str = "float32"):
    str_dtype_to_torch(dtype)
    ret = {}
    ...
    ret[f"{tbm_prefix}.linear"] = [{"weight": state_dict[f"{prefix}.linear.weight"], "bias": None}]
    ...
    for i in range(config.no_blocks):
        ...
        for name in ["tri_mul_out", "tri_mul_in"]:
            ...
            tbnm_state_dict.update(
                get_trimul_torch_weights(module_state_dict, original_prefix, tbm_prefix, dtype=config.dtype)
            )
        ...
        for name in ["start", "end"]:
            ...
            tbnm_state_dict.update(
                get_triattn_torch_weights(module_state_dict, original_prefix, tbm_prefix, dtype=config.dtype)
            )
...
def convert_hf_extra_msa_stack_torch(
    ...
    for i in range(config.no_blocks):
        ...
        tbnm_state_dict[f"blocks.{i}.msa_att_col.global_attention.fused_proj_kv"] = [
...
def convert_hf_template_embedder_torch(
    ...
    tbnm_state_dict["template_single_embedder.linear_1"] = [
        {
            "weight": module_state_dict[f"{prefix}.linear_1.weight"],
            "bias": module_state_dict[f"{prefix}.linear_1.bias"],
        }
    ]
    tbnm_state_dict["template_single_embedder.linear_2"] = [
        {
...
def convert_hf_template_embedder_multimer_torch(
    ...
    # Weight for template single embedder
    prefix = "template_single_embedder"
    tbnm_state_dict["template_single_embedder.template_single_embedder"] = [
        {
    ...
    tbnm_state_dict["template_single_embedder.template_projector"] = [
        {
...
def convert_hf_structure_module_torch(
    ...
    tbnm_state_dict["transition.layer_norm"] = [
        {
    ...
    tbnm_state_dict["ipa.head_weights"] = state_dict[f"{layer_path}ipa.head_weights"]
